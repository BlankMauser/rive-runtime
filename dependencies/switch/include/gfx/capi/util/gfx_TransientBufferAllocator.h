...
namespace nn{ namespace gfx{ namespace util{
    ...
    //! @brief  Allocator that manages the region in the buffer that is temporarily used.
    //! @details
    //!  Dynamically allocates the buffer region that is temporarily used during rendering.
    //!  Also provides a feature to deallocate all at the same time after rendering completes.
    class TransientBufferAllocator
    {
        NN_DISALLOW_COPY(TransientBufferAllocator);

    public:
        //! @brief  The maximum value for the size of a buffer that can be managed with this class.
        ...
        //! @details
        //!  The function returns a negative value when region allocation fails.
    ...
    public:
        ...
        //! @brief  Gets the buffer and the range for the offset to be managed, and then initializes.
        ...
        //! @pre  <tt><var>pBuffer</var></tt> must be a pointer to a valid buffer.
        ...
        //! @details
        //!  This object targets the region with the range specified in the buffer for management.
        //!  A contiguous valid range must be specified for the region.
        //!
        ...
        //! @brief  Determines whether allocation for the region from this object has started.
        //! @pre  <tt>IsInitialized()</tt> == <tt>true</tt>.
        //! @return  Returns <tt>true</tt> when region allocation has started. Returns <tt>false</tt> otherwise.
        //! @details
        //!  Success for <tt>Begin()</tt> triggers the start of region allocation.
        //!  <tt>End()</tt> triggers the end of region allocation.
        ...
        //! @brief  Gets the buffer configured for this object.
        //! @pre  <tt>IsInitialized()</tt> == <tt>true</tt>.
        //! @return  Returns the set buffer.
        //! @details
        //!  This function is thread-safe.
        nn::gfx::Buffer* GetBuffer() const NN_NOEXCEPT;

        //! @brief  Gets the offset from the start of the region in the buffer managed by this object.
        //! @pre  <tt>IsInitialized()</tt> == <tt>true</tt>.
        //! @return  Returns the offset from the start of the managed region.
        //! @details
        //!  This function is thread-safe.
        ptrdiff_t GetBaseOffset() const NN_NOEXCEPT;

        //! @brief  Gets the size, in bytes, for the region in the buffer managed by this object.
        //! @pre  <tt>IsInitialized()</tt> == <tt>true</tt>.
        //! @return  Returns the size, in bytes, for the managed region.
        //! @details
        //!  This function is thread-safe.
        size_t GetSize() const NN_NOEXCEPT;

        //! @brief  Gets the maximum alignment, in bytes, that can be used when allocating a region from this object.
        //! @pre  <tt>IsInitialized()</tt> == <tt>true</tt>.
        //! @return  Returns the maximum alignment, in bytes, that can be used when allocating a region.
        //! @details
        //!  This function is thread-safe.
        size_t GetAllocatableAlignmentMax() const NN_NOEXCEPT;

        //! @brief  Starts allocating the region in the buffer.
        //! @pre  <tt>IsInitialized()</tt> == <tt>true</tt>.
        ...
        //! @brief  Finishes allocating the region in the buffer.
        //! @pre  <tt>IsInitialized()</tt> == <tt>true</tt>.
        //! @pre  <tt>IsRecording()</tt> == <tt>true</tt>.
        //! @post  <tt>IsRecording()</tt> == <tt>false</tt>.
        //! @post  The range of the region allocated between calls to the immediately prior <tt>Begin()</tt> function and the <tt>End()</tt> function is recorded.
        //! @post  The value returned by <tt>GetHistoryLength()</tt> is incremented by 1.
        //! @details
        //!  The history is recorded even when there is no region allocated between calls to the immediately prior <tt>Begin()</tt> function and the <tt>End()</tt> function.
        //!  Calling this function on the same instance is not thread-safe.
        //!  Free()
        void End() NN_NOEXCEPT;

        //! @brief  Releases the oldest recorded region from among those recorded by this object.
        //! @pre  <tt>IsInitialized()</tt> == <tt>true</tt>.
        //! @pre  <tt>GetHistoryLength()</tt> > <tt>0</tt>.
        //! @post  Releases the oldest region recorded in the history.
        //! @post  The value returned by <tt>GetHistoryLength()</tt> is decremented by 1.
        //! @details
        //!  The region allocated between the calls to <tt>Begin()</tt> and <tt>End()</tt> is treated as a single unit and released.
        //!  The regions allocated individually using <tt>Allocate()</tt> cannot be released on an individual basis.
        //!  The released region becomes immediately available for allocation.
        //!  Calling this function multiple times on the same instance is not thread-safe.
        //!  Calling this function and the function that allocates regions at the same time on the same instance is thread-safe.
        //!  End()
        void Free() NN_NOEXCEPT;

        //! @brief  Allocates a region in the buffer.
        //! @param[in] size  The size, in bytes, of the region to allocate.
        //! @param[in] alignment  The alignment, in bytes, of the region to allocate.
        //! @pre  <tt>IsInitialized()</tt> == <tt>true</tt>.
        //! @pre  <tt>IsRecording()</tt> == <tt>true</tt>.
        //! @pre  <tt><var>alignment</var></tt> <= <tt>GetAllocatableAlignmentMax()</tt>.
        //! @pre  <tt><var>alignment</var></tt> must be a power of 2.
        //! @return  Returns the offset from the start of the allocated region.
        //!  Returns <tt>InvalidOffset</tt> when region allocation fails.
        //! @details
        ...
        //!  This function is thread-safe.
        //!  Can be called simultaneously with other functions allocating regions.
