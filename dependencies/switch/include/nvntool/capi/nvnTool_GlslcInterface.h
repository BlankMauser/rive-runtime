...
#define GLSLC_API_VERSION_MINOR    22

#define GLSLC_GPU_CODE_VERSION_MAJOR 1
#define GLSLC_GPU_CODE_VERSION_MINOR 16
...
/// A number indicating the start of a Graphic GPU program data section.
/// GLSLC_GPU_CODE_SECTION_DATA_MAGIC_NUMBER is left-over for compatibility reasons.
#define GLSLC_GPU_CODE_SECTION_DATA_MAGIC_NUMBER 0x12345678U
#define GLSLC_GFX_GPU_CODE_SECTION_DATA_MAGIC_NUMBER GLSLC_GPU_CODE_SECTION_DATA_MAGIC_NUMBER

/// A number indicating the start of a Compute GPU program data section.
#define GLSLC_COMP_GPU_CODE_SECTION_DATA_MAGIC_NUMBER 0x12345679U
...
typedef struct GLSLCoptionFlags_rec {
    ...
    /// NOTE: Do NOT use this option if using NVN's viewport swizzling feature
    //  (i.e.  nvn::CommandBuffer::SetViewportSwizzle).  The CBF 
    //  optimization produces shader code incompatible with viewport swizzles.
    uint32_t enableCBFOptimization : 1;

    /// Option to compile the shader using the warp culling optimization
    /// Refer to the GLSLC programming guide for more details
    /// NOTE: Do NOT use this option if using NVN's viewport swizzling feature
    //  (i.e.  nvn::CommandBuffer::SetViewportSwizzle).  The warp culling
    //  optimization produces shader code incompatible with viewport swizzles.
    ...
    /// Option to indicate that GLSLC should ignore bindings assigned to resources in the shader
    /// (either from using layout(binding = ... ) or binding annotations in the SPIR-V binaries).  GLSLC
    /// will instead automatically assign bindings to the resources in the shader using the binding
    /// assignment policy that it would use if no bindings were written in the source shaders (see
    /// the GLSLC programming guide for details on this binding assignment policy).  Developers must
    /// use reflection information to determine which bindings were assigned to resources and can not
    /// rely on explicit bindings in the shader.
    uint32_t ignoreBindings : 1;
} GLSLCoptionFlags;
...
typedef struct GLSLCgpuCodeHeader_rec {
    ...
    /// NX device.
    ///
    /// \note The Windows reference NVN driver manages its own internal scratch
    /// memory allocations for the target desktop GPU and will ignore the
    /// scratch memory provided by applications.
    ///
...
/// If it has not been called, then this function will do a full compile.
///
/// NOTE: Though glslcCompileSpecializedMT is thread safe, it does not mean it will run in parallel.
/// In the Windows implementation, this function will take a CriticalSection at the beginning of the function,
/// unlock in the middle to run in parallel, and then do another CriticalSection lock/release
/// at the end of this function.
/// As result, if many threads call glslccompileSpecializedMT at once, most of the threads will simply be waiting
/// for the release of the CriticalSection and thus does not contribute to parallelism.
/// Also, in current implementation of the CriticalSection by the Windows, we see very long
/// overhead between one thread unlocking the CriticalSection and other thread waiting for the CriticalSection
/// having the CPU assigned, even while most of the CPU/HT is shown idle from the Task Manager.
/// And this overhead will grow as number of theads waiting for the same CriticalSection grow.
/// So, for the current implementation, we recommend not to let many threads to call this API at once.
/// Our recommendation is at most three threads. This will result in one thread running the code range
/// locked by CriticalSection, one thread running the intermediate CriticalSection free part,
/// and last thread waiting for the release of the CriticalSection.
///
/// \param compileObject The previously initialized GLSLCcompileObject.
/// \param specEntries A pointer to the GLSLCspecializationBatch which contains 1 or more GLSLCspecializationSets
