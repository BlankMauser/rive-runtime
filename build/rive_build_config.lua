...
newoption({
    trigger = 'nvn_bind_logs',
    description = 'enable NVN bind call logging',
})
newoption({
    trigger = 'nvn_reflection_logs',
    description = 'enable NVN shader reflection logging',
})
newoption({
    trigger = 'nvn_pls_binding_offset',
    value = 'value',
    description = 'override NVN PLS image binding base offset',
})
newoption({
    trigger = 'nvn_pls_color_binding',
    value = 'value',
    description = 'override NVN PLS color binding index',
})
newoption({
    trigger = 'nvn_pls_clip_binding',
    value = 'value',
    description = 'override NVN PLS clip binding index',
})
newoption({
    trigger = 'nvn_pls_scratch_binding',
    value = 'value',
    description = 'override NVN PLS scratch binding index',
})
newoption({
    trigger = 'nvn_pls_coverage_binding',
    value = 'value',
    description = 'override NVN PLS coverage binding index',
})
newoption({
    trigger = 'nvn_pls_color_format',
    value = 'value',
    description = 'override NVN PLS color format (decimal or 0x...)',
})
newoption({
    trigger = 'nvn_pls_clip_format',
    value = 'value',
    description = 'override NVN PLS clip format (decimal or 0x...)',
})
newoption({
    trigger = 'nvn_pls_scratch_format',
    value = 'value',
    description = 'override NVN PLS scratch format (decimal or 0x...)',
})
newoption({
    trigger = 'nvn_pls_coverage_format',
    value = 'value',
    description = 'override NVN PLS coverage format (decimal or 0x...)',
})
newoption({
    trigger = 'nvn_pls_coverage_clear',
    value = 'value',
    description = 'override NVN PLS coverage clear value (decimal or 0x...)',
})
...
if _OPTIONS['nvn_bind_logs'] then
    defines({ 'RIVE_NVN_ENABLE_BIND_LOGS=1' })
end
if _OPTIONS['nvn_reflection_logs'] then
    defines({ 'RIVE_NVN_ENABLE_REFLECTION_LOGS=1' })
end
if _OPTIONS['nvn_pls_binding_offset'] then
    local offset = tonumber(_OPTIONS['nvn_pls_binding_offset'])
    if not offset then
        error('nvn_pls_binding_offset must be a number')
    end
    defines({ 'RIVE_NVN_PLS_BINDING_BASE=' .. offset })
end
if _OPTIONS['nvn_pls_color_binding'] then
    local value = tonumber(_OPTIONS['nvn_pls_color_binding'])
    if not value then
        error('nvn_pls_color_binding must be a number')
    end
    defines({ 'RIVE_PLS_COLOR_BINDING=' .. value })
end
if _OPTIONS['nvn_pls_clip_binding'] then
    local value = tonumber(_OPTIONS['nvn_pls_clip_binding'])
    if not value then
        error('nvn_pls_clip_binding must be a number')
    end
    defines({ 'RIVE_PLS_CLIP_BINDING=' .. value })
end
if _OPTIONS['nvn_pls_scratch_binding'] then
    local value = tonumber(_OPTIONS['nvn_pls_scratch_binding'])
    if not value then
        error('nvn_pls_scratch_binding must be a number')
    end
    defines({ 'RIVE_PLS_SCRATCH_BINDING=' .. value })
end
if _OPTIONS['nvn_pls_coverage_binding'] then
    local value = tonumber(_OPTIONS['nvn_pls_coverage_binding'])
    if not value then
        error('nvn_pls_coverage_binding must be a number')
    end
    defines({ 'RIVE_PLS_COVERAGE_BINDING=' .. value })
end
local function parse_numeric_option(raw, label)
    if raw == nil then
        return nil
    end
    local value = nil
    ...
    if value == nil then
        error(label .. ' must be a number (decimal or 0x...)')
    end
    return value
end
if _OPTIONS['nvn_pls_color_format'] then
    local value = parse_numeric_option(_OPTIONS['nvn_pls_color_format'], 'nvn_pls_color_format')
    defines({ 'RIVE_NVN_PLS_COLOR_FORMAT=' .. value })
end
if _OPTIONS['nvn_pls_clip_format'] then
    local value = parse_numeric_option(_OPTIONS['nvn_pls_clip_format'], 'nvn_pls_clip_format')
    defines({ 'RIVE_NVN_PLS_CLIP_FORMAT=' .. value })
end
if _OPTIONS['nvn_pls_scratch_format'] then
    local value = parse_numeric_option(_OPTIONS['nvn_pls_scratch_format'], 'nvn_pls_scratch_format')
    defines({ 'RIVE_NVN_PLS_SCRATCH_FORMAT=' .. value })
end
if _OPTIONS['nvn_pls_coverage_format'] then
    local value = parse_numeric_option(_OPTIONS['nvn_pls_coverage_format'], 'nvn_pls_coverage_format')
    defines({ 'RIVE_NVN_PLS_COVERAGE_FORMAT=' .. value })
end
if _OPTIONS['nvn_pls_coverage_clear'] then
    local value = parse_numeric_option(_OPTIONS['nvn_pls_coverage_clear'], 'nvn_pls_coverage_clear')
    defines({ 'RIVE_NVN_PLS_COVERAGE_CLEAR_VALUE=' .. value })
end
