...
struct DeviceInfo {
    ...
    enum Enum {
        ...
        SHADER_SUBGROUP_SIZE = 98,

        /// \brief Maximum size (in bytes) of dynamic checkpoint marker
        /// data.
        ///
        /// This value defines the maximum size in bytes that can be
        /// submitted as dynamic checkpoint marker data buffer.
        CHECKPOINT_MAXIMUM_DYNAMIC_MARKER_DATA_SIZE = 99,

        /// \brief Maximum number of call stack entries that can be
        /// captured for checkpoint event markers.
        ///
        /// This value defines the maximum number of call stack entries
        /// that can be captured for checkpoint event markers.
        CHECKPOINT_MAXIMUM_CALL_STACK_CAPTURE_DEPTH = 100,

        /// \brief Maximum supported length of the event sampling interval
        /// for automatic checkpoint event markers.
        CHECKPOINT_MAXIMUM_SAMPLING_INTERVAL_LENGTH = 101,

        /// \brief Maximum size (in bytes) of static checkpoint marker
        /// data.
        ///
        /// This value defines the maximum size in bytes that can be
        /// submitted as static checkpoint marker data buffer.
        CHECKPOINT_MAXIMUM_STATIC_MARKER_DATA_SIZE = 102,

        /// \brief Minimum alignment of shader code bindings.
        SHADER_CODE_ALIGNMENT = 103,
        ...
        VERTEX_ATTRIBUTE_STATE_MAXIMUM_RELATIVE_OFFSET = 104,
...
struct DeviceFlagBits {
    ...
    enum Enum {
        ...
        /// the device.
        ///
        /// Enables minimal debug layer features.  Object pointer
        /// validation is not performed.  Only performs simple parameter
        /// checks, and invalid API usage, cases where the driver would
        /// return FALSE.  If the debug layer is enabled on NX, the
        /// application must call nv::SetGraphicsDevtoolsAllocator prior to
        /// nvnDeviceInitialize to provide an allocator that can be used by
        /// the debug layer.
        ...
        /// underlying memory pool is finalized.
        ...
        /// dispatch, at queue submit time.  Draw-time validation will
        /// catch common mistakes that would otherwise lead to hardware
        /// errors or hangs, but the additional state tracking required may
        ...
        /// buffer command memory being reused.  This will enable the most
        /// checks in debug layer, but also has the most impact on
        /// performance.
...
struct DebugCallbackSource {
    ...
    enum Enum {
        ...
        /// \brief A debug callback is being issued while initializing the
        /// NVN driver.
        ///
        /// \note To get notification of errors during the initialization
        /// phase a special debug callback must have been installed using
        /// nvnInstallGlobalDebugCallback.
        INITIALIZATION = 0x00000002,
...
struct DebugCallbackType {
    ...
    enum Enum {
        ...
        /// \brief A debug callback is being issued because of an error
        /// during driver initialization.
        ///
        /// \note To get notification of errors during the initialization
        /// phase a special debug callback must have been installed using
        /// nvnInstallGlobalDebugCallback.
        INITIALIZATION_ERROR = 0x00000004,
...
struct MemoryPoolFlags {
    ...
    enum Enum {
        ...
        ///
        /// \note For graphics developer tools to be able to access the
        /// shader program at run-time, memory pools holding shader code
        /// should not have the flag nvn::MemoryPoolFlags::CPU_NO_ACCESS.
        /// Failure to adhere to this may prevent developer tools from
        /// working properly.
        ...
        /// be initialized with GPU_NO_ACCESS set.  When mapping memory
        /// into virtual pools, the pools providing memory must have been
        ...
        /// Maxwell GPUs) or undefined values.  After a virtual memory pool
        /// is initialized, it can be populated with pages from physical
        /// pools using MemoryPool::MapVirtual.  Virtual pools are not
        /// directly accessible by the CPU and must be initialized with
        /// CPU_NO_ACCESS set.
        ///
...
struct CounterType {
    ...
    enum Enum {
        ...
        /// be processed only once.  Additionally, long strip primitives
        /// may be split into batches, where shared vertices are processed
        /// multiple times.
        ...
        /// timestamp:
        ///
        /// \li Word 0:  the number of 8x8 tiles processed by ZCull
        /// (whether culled or not)
        ...
        /// \li Word 3:  the number of 4x4 pixel blocks that were culled
        /// due to failing the stencil test.
...
struct TextureFlags {
    ...
    enum Enum {
        ...
        /// in the ZCull memory.  Note that enabling stencil culling
        /// results in a reduced precision of the depth culling.
...
struct BarrierBits {
    ...
    enum Enum {
        ...
        /// older GPUs, there are a few exceptions to the ordering
        /// guarantee for ORDER_FRAGMENTS.  The GPU may begin processing
        /// depth/stencil clears or depth/stencil testing (including buffer
        /// updates) for fragments after the barrier before completing all
        /// processing of fragments before the barrier.  When fragments
        /// from after the barrier perform such updates to a texture not
        /// yet read by fragments from before the barrier, a heavier
        /// barrier like ORDER_PRIMITIVES is necessary to ensure the "new"
        /// fragments don't overwrite values not yet read by "old" ones.
...
struct CopyFlags {
    ...
    enum Enum {
        ...
        /// CommandBuffer::CopyBufferToTexture.  This flag should not be
        /// set for any other copy command.
        ///
...
struct EventSignalLocation {
    ...
    enum Enum {
        ...
        /// completed work on all previously submitted draw calls.  Does
        /// not wait for rasterization and fragment processing.
        VERTEX_PIPE = 1,
...
struct SampleShadingMode {
    ...
    enum Enum {
        ...
        /// \brief Default mode.  The fragment shader is evaluated either
        /// by fragment or by sample, depending on whether a keyword such
        /// as gl_SamplePosition is specified within the fragment shader.
        DEFAULT_FRAGMENT = 0,
...
/// The data structure is for one shader stage.  The contents of this
/// struct contain everything NVN needs to set up the program for use with
/// Program::SetShaders.  This is an extended version of NVNshaderData
...
struct AutomaticCheckpointsMask {

    /// \brief Enumerated values for the type AutomaticCheckpointsMask.
    enum Enum {

        /// \brief Enables automatic checkpoint event markers for all draw
        /// commands.
        ///
        /// If set, this flag indicates that automatic checkpoint event
        /// marker should be added for all draw commands.
        DRAW = 0x00000001,

        /// \brief Enables automatic checkpoint event markers for all
        /// dispatch compute commands.
        ///
        /// If set, this flag indicates that automatic checkpoint event
        /// marker should be added for all dispatch compute commands.
        COMPUTE_DISPATCH = 0x00000002,

        /// \brief Enables automatic checkpoint event markers for all copy
        /// commands.
        ///
        /// If set, this flag indicates that automatic checkpoint event
        /// marker should be added for all copy commands.
        COPY = 0x00000004,
        ...
        ///
        /// If set, this flag indicates that automatic checkpoint event
        /// marker should be added for all debug group push commands.
        DEBUG_GROUP = 0x00000008,

        /// \brief Enables automatic checkpoint event markers for all
        /// supported commands.
        ///
        /// If set, this flag indicates that automatic checkpoint event
        /// marker should be added for all supported commands.  This is the
        /// default setting.
        ALL = 0x0fffffff,

        NVN_ENUM_32BIT(AUTOMATIC_CHECKPOINTS_MASK),
    };
    NVN_BITFIELD_CLASS(AutomaticCheckpointsMask, NVNautomaticCheckpointsMask);
};
...
/// DeviceInfo::COMMAND_BUFFER_MIN_CONTROL_SIZE.  When building command
/// sets from other command sets using CommandBuffer::CopyCommands, this
...
typedef void (NVNAPIENTRYP InstallGlobalDebugCallbackFunc)(const DebugCallbackFunc callback,
                                                           void *callbackData);
...
public:
    ...
    /// Fastpath variant.
    ///
    /// \note:  This is functionally equivalent to DeviceBuilder::GetFlags,
    /// but should be faster in common cases.  Not compatible with the NVN
    /// debug layer.
    inline DeviceFlagBits GetFlags_fastpath() const;
...
public:
    ...
    ///
    /// Annotates a Device object with a descriptive text label.  This
    ...
    /// ReportCounter functions.  See the \ref nvn_guide_counters
    /// "Counters" section of the NVN Programming Guide for more detail.
    ///
    ...
    /// intermediate shader cache can hold.  If this is 0, the cache will
    /// be disabled and any previous cache resources will be cleaned up.
    /// If a non-zero value is used, the cache will hold intermediate code
    /// for up to _numMaxEntries_ unique shaders.  Programs containing
    /// multiple shader stages will consume multiple entries in this cache.
    ...
    /// creating any queues, command buffers, or windows, and should use
    /// that mode for the lifetime of the application.  Rendering results
    /// are undefined if there are any inconsistencies between the window
    /// origin mode (a) currently set for the device, (b) set when creating
    /// the command buffer used to create rendering commands, (c) set when
    /// creating the queue used to submit rendering commands, (d) set when
    /// creating the window used to display rendered frames.
    ///
    ...
    /// Device::SetWindowOriginMode, but should be faster in common cases.
    /// Not compatible with the NVN debug layer.
    ///
    /// \param windowOriginMode New location for the window origin.
    inline void SetWindowOriginMode_fastpath(WindowOriginMode windowOriginMode);
    ...
    /// the queue used to submit rendering commands.
    ///
    /// \note:  This is functionally equivalent to Device::SetDepthMode,
    /// but should be faster in common cases.  Not compatible with the NVN
    /// debug layer.
    ///
    /// \param depthMode New policy for depth clipping.
    inline void SetDepthMode_fastpath(DepthMode depthMode);
    ...
    /// use for future depth buffer clears.  Clearing the depth buffer
    /// using one of these registered clear depth values will result in
    /// reduced memory bandwidth usage and increased performance when used
    ...
    /// \brief Query the window origin mode for a device.  Fastpath
    /// variant.
    ///
    /// \note:  This is functionally equivalent to
    /// Device::GetWindowOriginMode, but should be faster in common cases.
    /// Not compatible with the NVN debug layer.
    inline WindowOriginMode GetWindowOriginMode_fastpath() const;
    ...
    ///
    /// \note:  This is functionally equivalent to Device::GetDepthMode,
    /// but should be faster in common cases.  Not compatible with the NVN
    /// debug layer.
    inline DepthMode GetDepthMode_fastpath() const;
    ...
    /// The control memory has a tracking token used to implicit cleanup
    /// the memory later even if the application does not call finalize.
    /// Overwriting the command set memory before calling
    /// FinalizeCommandHandle or implicit clean can corrupt the tracking
    /// token and may result in memory leaks.
    ...
    ///
    /// If automatic checkpoint event markers are enabled, a checkpoint
    /// marker is added automatically for draw, dispatch, or copy commands
    /// recorded into a command set.  In the case of a GPU crash, the
    /// checkpoint event markers that were last processed by the GPU will
    /// be included in NVN Aftermath crash dumps to aid in narrowing down
    /// the command that has led to the crash.
    /// If automatic checkpoints are sucessfully enabled, NVN_TRUE is
    /// returned.  If any of the passed in parameters is invalid or a
    /// problem occurs when enabling automatic checkpoints, NVN_FALSE is
    /// returned and the currently set state remains unchanged.
    ///
    /// \note This function is a no-op if the application is not linked
    /// with the NVN Aftermath target library.
    /// \note Calling this function while a command buffer is being
    /// recorded on the device will result in undefined behavior.
    ///
    /// \param enable Enable automatic checkpoint event markers if true;
    /// disable if false.  Automatic checkpoint event markers are disabled
    /// by default.
    inline NVNboolean SetAutomaticCheckpointsEnable(NVNboolean enable);

    /// \brief Control for which commands automatic checkpoint event
    /// markers are added.
    ///
    /// If automatic checkpoint event markers are enabled, the mask set by
    /// this function specifies for which subset of the commands automatic
    /// checkpoints are added.
    /// If the mask is sucessfully set, NVN_TRUE is returned.  If any of
    /// the passed in parameters is invalid or a problem occurs when
    /// setting the mask, NVN_FALSE is returned and the currently set state
    /// remains unchanged.
    /// \note This function is a no-op if the application is not linked
    /// with the NVN Aftermath target library.
    /// \note Calling this function while a command buffer is being
    ...
    /// nvn::AutomaticCheckpointsMask.
    inline NVNboolean SetAutomaticCheckpointsMask(AutomaticCheckpointsMask mask);

    /// \brief Control how frequently automatic checkpoint event markers
    /// are inserted.
    ///
    /// If automatic checkpoint event markers are enabled, this function
    /// specifies how frequently they are added.
    /// If the sampling interval is set sucessfully, NVN_TRUE is returned.
    /// If any of the passed in parameters is invalid or a problem occurs
    /// when setting the sampling interval, NVN_FALSE is returned and the
    /// currently set sampling interval remains unchanged.
    ///
    /// \note This function is a no-op if the application is not linked
    /// with the NVN Aftermath target library.
    /// \note Calling this function while a command buffer is being
    /// recorded on the device will result in undefined behavior.
    ///
    /// \param interval A value of n will insert a checkpoint marker only
    /// on every n-th command.
    /// A value of n will insert a checkpoint only on every n-th command.
    /// The default value for the sampling interval is 1, which means a
    /// checkpoint will be inserted on every draw, dispatch, or copy
    /// command.
    /// Must be greater than zero.
    /// Must be less than or equal to 65535 (the value of
    /// nvn::DeviceInfo::CHECKPOINT_MAXIMUM_SAMPLING_INTERVAL_LENGTH).
    inline NVNboolean SetAutomaticCheckpointsSamplingInterval(int interval);

    /// \brief Control automatic checkpoint event markers by debug group.
    ///
    /// Specifies that automatic checkpoint event markers should only be
    /// added if the name of the debug group at the top of the selected
    /// debug group stack matches the provided debug group name.
    /// If the debug group name filter is set sucessfully, NVN_TRUE is
    /// returned.  If any of the passed in parameters is invalid or a
    /// problem occurs when setting the debug group name filter, NVN_FALSE
    /// is returned and the currently set debug group name filter remains
    /// unchanged.
    ///
    /// \note This function is a no-op if the application is not linked
    /// with the NVN Aftermath target library.
    ...
    ///
    /// \param debugGroupName Name of the debug group for which automatic
    /// event markers are inserted.  Call with nullptr to disable the
    /// filtering by debug group name.  The default setting is to apply no
    /// filter.
    inline NVNboolean SetAutomaticCheckpointsByDebugGroup(uint32_t domainId,
                                                          const char * debugGroupName);

    /// \brief Control call stack capture for checkpoint event markers.
    ///
    /// If call stack capture for checkpoint event markers is enabled, the
    /// CPU call stack for every function call inserting event markers
    /// (user or automatic) is captured and is associated as additional
    /// payload with the event marker.  In the case of a GPU crash this
    /// data is also added into the NVN Aftermath crash dump and can
    /// provide additional hints about the origin of the problem.
    /// If call stack capture is sucessfully enabled, NVN_TRUE is returned.
    ///  If any of the passed in parameters is invalid or a problem occurs
    /// when enabling call stack capture, NVN_FALSE is returned and the
    /// currently set state remains unchanged.
    ///
    /// \note This function is a no-op if the application is not linked
    /// with the NVN Aftermath target library.
    /// \note Calling this function while a command buffer is being
    /// recorded on the device will result in undefined behavior.
    ///
    /// \param enable Enable or disable call stack capture for checkpoint
    /// event markers.  Call stack capture is disabled by default.
    inline NVNboolean SetCheckpointCallStacksEnable(NVNboolean enable);

    /// \brief Control call stack capture depth for checkpoint event
    /// markers.
    ///
    ...
    /// returned.  If any of the passed in parameters is invalid or a
    /// problem occurs when setting the call stack capture depth, NVN_FALSE
    /// is returned and the currently set value remains unchanged.
    ///
    /// \note This function is a no-op if the application is not linked
    /// with the NVN Aftermath target library.
    /// \note Calling this function while a command buffer is being
    /// recorded on the device will result in undefined behavior.
    ///
    /// \param depth Maximum depth of call stack capture.  The default
    /// value for the call stack capture depth is 8 levels.
    /// Must be greater than zero.
    /// Must be less than or equal to 16 (the value of
    /// nvn::DeviceInfo::CHECKPOINT_MAXIMUM_CALL_STACK_CAPTURE_DEPTH).
    inline NVNboolean SetCheckpointMaxCallStacksDepth(int depth);
...
public:
    ...
    /// drain.  On the other hand, unnecessarily large buffer wastes
    /// memory.  The default size of 16KB should be enough for most
    /// applications, but an application may want to tune this value for
    /// its needs.  NVN will issue a debug layer performance warning in
    ...
    /// \brief Set a priority on a QueueBuilder object.  Fastpath variant.
    ///
    /// Setting a priority on a QueueBuilder object allows for additional
    /// control of the relative execution timing between Queues.  Queue
    /// Priority affects both the execution order of the Queues in their
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// QueueBuilder::SetQueuePriority, but should be faster in common
    /// cases.  Not compatible with the NVN debug layer.
    ///
    /// \param priority The priority to set on the queue.
    inline QueueBuilder & SetQueuePriority_fastpath(const QueuePriority priority);
    ...
    /// Fastpath variant.
    ///
    /// \note:  This is functionally equivalent to QueueBuilder::GetDevice,
    /// but should be faster in common cases.  Not compatible with the NVN
    /// debug layer.
    inline const Device * GetDevice_fastpath() const;
    ...
    /// Fastpath variant.
    ///
    /// \note:  This is functionally equivalent to QueueBuilder::GetFlags,
    /// but should be faster in common cases.  Not compatible with the NVN
    /// debug layer.
    inline int GetFlags_fastpath() const;
    ...
    /// Fastpath variant.
    ///
    /// \note:  This is functionally equivalent to QueueBuilder::GetMemory,
    /// but should be faster in common cases.  Not compatible with the NVN
    /// debug layer.
    inline void * GetMemory_fastpath() const;
...
public:
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// CommandBuffer::SetCommandMemoryCallbackEnabled, but should be
    /// faster in common cases.  Not compatible with the NVN debug layer.
    ///
    /// \param enabled True (default) if the out-of-memory callback should
    /// be enabled; false otherwise.
    inline void SetCommandMemoryCallbackEnabled_fastpath(NVNboolean enabled);

    /// \brief Add command data memory for a CommandBuffer object.  This
    /// can be called at any time, even outside an out-of-memory callback
    /// or while recording or not recording.  Any leftover space in the
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// CommandBuffer::BeginRecording, but should be faster in common
    /// cases.  Not compatible with the NVN debug layer.
    inline void BeginRecording_fastpath();
    ...
    inline void CallCommands(int numCommands,
                             NVN_NOESCAPE const CommandHandle *handles);
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// CommandBuffer::CallCommands, but should be faster in common cases.
    /// Not compatible with the NVN debug layer.
    ///
    /// \param numCommands Number of command sets to call.
    ///
    /// \param handles Array of numCommands command set handles to call.
    inline void CallCommands_fastpath(int numCommands,
                                      NVN_NOESCAPE const CommandHandle *handles);
    ...
    inline void BindVertexAttribState(int numAttribs,
                                      NVN_NOESCAPE const VertexAttribState *attribs);
    ...
    inline void BindVertexStreamState(int numStreams,
                                      NVN_NOESCAPE const VertexStreamState *streams);
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// CommandBuffer::BindVertexBuffer, but should be faster in common
    /// cases.  Not compatible with the NVN debug layer.
    ///
    /// \param index Vertex buffer binding point number to update.
    /// Must be less than 16 (the value of
    ...
    /// \param size Size (in bytes) of the range of the Buffer object to
    /// bind.
    inline void BindVertexBuffer_fastpath(int index,
                                          BufferAddress buffer,
                                          size_t size);
    ...
    inline void BindVertexBuffers(int first,
                                  int count,
                                  NVN_NOESCAPE const BufferRange *buffers);

    /// \brief Bind one or more Buffer object ranges to use for fetching
    /// vertex attributes.  Fastpath variant.
    ///
    /// \note:  This is functionally equivalent to
    /// CommandBuffer::BindVertexBuffers, but should be faster in common
    /// cases.  Not compatible with the NVN debug layer.
    ///
    /// \param first First vertex buffer binding point number to update.
    /// Must be less than 16 (the value of
    ...
    ///
    /// \param buffers Array of vertex buffer ranges to bind.
    inline void BindVertexBuffers_fastpath(int first,
                                           int count,
                                           NVN_NOESCAPE const BufferRange *buffers);
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// CommandBuffer::BindUniformBuffer, but should be faster in common
    /// cases.  Not compatible with the NVN debug layer.
    ///
    /// \param stage Shader stage owning the binding point to update.
    ///
    ...
    /// Must be less than or equal to 65536 (the value of
    /// nvn::DeviceInfo::MAX_UNIFORM_BUFFER_SIZE).
    inline void BindUniformBuffer_fastpath(ShaderStage stage,
                                           int index,
                                           BufferAddress buffer,
                                           size_t size);
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// CommandBuffer::BindUniformBuffers, but should be faster in common
    /// cases.  Not compatible with the NVN debug layer.
    ///
    /// \param stage Shader stage owning the binding point to update.
    ///
    ...
    ///
    /// \param buffers Array of buffer object ranges to bind.
    inline void BindUniformBuffers_fastpath(ShaderStage stage,
                                            int first,
                                            int count,
                                            NVN_NOESCAPE const BufferRange *buffers);
    ...
    /// shader uniform values.
    ///
    /// When recording this command, the pointer to the set of buffer
    /// object ranges is not dereferenced when the command set is recorded.
    ///  Instead, it is dereferenced only when the command set containing
    /// this command is submitted to a queue.
    ///
    /// \param stage Shader stage owning the binding point to update.
    ///
    /// \param first First uniform buffer binding point number to update.
    ...
    ///
    /// \param buffers Array of buffer object ranges to bind.
    inline void BindUniformBuffersDeferred(ShaderStage stage,
                                           int first,
                                           int count,
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// CommandBuffer::BindUniformBuffersDeferred, but should be faster in
    ...
    ///
    /// \param buffers Array of buffer object ranges to bind.
    inline void BindUniformBuffersDeferred_fastpath(ShaderStage stage,
                                                    int first,
                                                    int count,
                                                    const BufferRange *buffers);
    ...
    ///
    /// \param buffers Array of buffer object ranges to bind.
    inline void BindStorageBuffersDeferred(ShaderStage stage,
                                           int first,
                                           int count,
                                           const BufferRange *buffers);
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// CommandBuffer::BindStorageBuffersDeferred, but should be faster in
    /// common cases.  Not compatible with the NVN debug layer.
    ///
    /// \param stage Shader stage owning the binding point to update.
    ///
    ...
    ///
    /// \param buffers Array of buffer object ranges to bind.
    inline void BindStorageBuffersDeferred_fastpath(ShaderStage stage,
                                                    int first,
                                                    int count,
                                                    const BufferRange *buffers);
    ...
    inline void BindTextures(ShaderStage stage,
                             int first,
                             int count,
                             NVN_NOESCAPE const TextureHandle *textures);

    /// \brief Bind one or more Texture/Sampler pairs to use for texture
    /// mapping in shaders.
    ///
    /// When recording this command, the pointer to the set of texture
    /// handles is not dereferenced when the command set is recorded.
    /// Instead, it is dereferenced only when the command set containing
    /// this command is submitted to a queue.
    ///
    /// \param stage Shader stage owning the binding points to update.
    ...
    /// \param textures Array of handles for pairs of Texture and Sampler
    /// objects to bind.
    inline void BindTexturesDeferred(ShaderStage stage,
                                     int first,
                                     int count,
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// CommandBuffer::BindTexturesDeferred, but should be faster in common
    /// cases.  Not compatible with the NVN debug layer.
    ///
    /// \param stage Shader stage owning the binding points to update.
    ///
    ...
    /// \param textures Array of handles for pairs of Texture and Sampler
    /// objects to bind.
    inline void BindTexturesDeferred_fastpath(ShaderStage stage,
                                              int first,
                                              int count,
                                              const TextureHandle *textures);
    ...
    inline void BindImages(ShaderStage stage,
                           int first,
                           int count,
                           NVN_NOESCAPE const ImageHandle *images);

    /// \brief Bind one or more Textures to use for image loads and stores
    /// in shaders.
    ///
    /// When recording this command, the pointer to the set of image
    /// handles is not dereferenced when the command set is recorded.
    /// Instead, it is dereferenced only when the command set containing
    /// this command is submitted to a queue.
    ///
    ...
    ///
    /// \param images Array of handles for Texture objects to bind.
    inline void BindImagesDeferred(ShaderStage stage,
                                   int first,
                                   int count,
                                   const ImageHandle *images);

    /// \brief Bind one or more Textures to use for image loads and stores
    /// in shaders.  Fastpath variant.
    ///
    /// When recording this command, the pointer to the set of image
    /// handles is not dereferenced when the command set is recorded.
    /// Instead, it is dereferenced only when the command set containing
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// CommandBuffer::BindImagesDeferred, but should be faster in common
    /// cases.  Not compatible with the NVN debug layer.
    ///
    /// \param stage Shader stage owning the binding points to update.
    ///
    ...
    ///
    /// \param images Array of handles for Texture objects to bind.
    inline void BindImagesDeferred_fastpath(ShaderStage stage,
                                            int first,
                                            int count,
                                            const ImageHandle *images);
    ...
    inline void SetInnerTessellationLevels(NVN_NOESCAPE const float *levels);
    ...
    inline void SetOuterTessellationLevels(NVN_NOESCAPE const float *levels);
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// CommandBuffer::DrawElements, but should be faster in common cases.
    /// Not compatible with the NVN debug layer.
    ///
    /// \param mode Type of the primitive to render.
    ///
    ...
    /// \param indexBuffer GPU address of buffer object memory used as the
    /// index buffer.
    inline void DrawElements_fastpath(DrawPrimitive mode,
                                      IndexType type,
                                      int count,
                                      BufferAddress indexBuffer);
    ...
    ///
    /// \warning On NX and the Windows reference implementation for Turing
    /// and older GPUs, calling CommandBuffer::Barrier with the barrier
    ...
    inline void SetViewportSwizzles(int first,
                                    int count,
                                    NVN_NOESCAPE const ViewportSwizzle *swizzles);
    ...
    /// \brief Bind a separate Texture to use for texture mapping in
    /// shaders.  Deprecated in favor of
    /// CommandBuffer::BindSeparateTexture.
    ///
    /// This command binds a separate texture handle (without sampler
    /// information) for use with shaders using separate texture types.
    ...
    inline void BindSeparateTextures(ShaderStage stage,
                                     int first,
                                     int count,
                                     NVN_NOESCAPE const SeparateTextureHandle *textureHandles);
    ...
    inline void BindSeparateSamplers(ShaderStage stage,
                                     int first,
                                     int count,
                                     NVN_NOESCAPE const SeparateSamplerHandle *samplerHandles);
    ...
    /// is equivalent to making multiple calls to BindSeparateTexture.
    ///
    /// When recording this command, the pointer to the set of separate
    /// texture handles is not dereferenced when the command set is
    /// recorded.  Instead, it is dereferenced only when the command set
    /// containing this command is submitted to a queue.
    ///
    /// \param stage Shader stage owning the binding points to update.
    ///
    /// \param first First separate texture binding point number to update.
    ...
    /// \param textureHandles Array of handles of separate Texture objects
    /// to bind.
    inline void BindSeparateTexturesDeferred(ShaderStage stage,
                                             int first,
                                             int count,
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// CommandBuffer::BindSeparateTexturesDeferred, but should be faster
    ...
    /// \param textureHandles Array of handles of separate Texture objects
    /// to bind.
    inline void BindSeparateTexturesDeferred_fastpath(ShaderStage stage,
                                                      int first,
                                                      int count,
                                                      const SeparateTextureHandle *textureHandles);
    ...
    ///
    /// When recording this command, the pointer to the set of separate
    /// sampler handles is not dereferenced when the command set is
    /// recorded.  Instead, it is dereferenced only when the command set
    /// containing this command is submitted to a queue.
    ///
    /// \param stage Shader stage owning the binding points to update.
    ...
    /// \param samplerHandles Array of handles of separate Sampler objects
    /// to bind.
    inline void BindSeparateSamplersDeferred(ShaderStage stage,
                                             int first,
                                             int count,
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// CommandBuffer::BindSeparateSamplersDeferred, but should be faster
    /// in common cases.  Not compatible with the NVN debug layer.
    ///
    /// \param stage Shader stage owning the binding points to update.
    ///
    ...
    /// \param samplerHandles Array of handles of separate Sampler objects
    /// to bind.
    inline void BindSeparateSamplersDeferred_fastpath(ShaderStage stage,
                                                      int first,
                                                      int count,
                                                      const SeparateSamplerHandle *samplerHandles);
    ...
    /// the stencil mask will not be updated by the stencil test.  There
    /// are separate stencil masks for front- and back-facing primitives.
    /// This function can update the value of either or both masks.  If
    /// stencil testing is disabled, the stencil mask has no effect.  The
    /// default value for both front and back masks is 0xFF (all bits set).
    ///
    /// \note In API versions prior to 53.8, the default mask was
    /// unintentionally set to zero.
    ///
    ...
    /// values.
    inline void SetBlendColor(NVN_NOESCAPE const float *blendColor);
    ...
    /// rasterization.  This function is only supported on
    /// second-generation Maxwell and later GPUs.  Query the
    /// SUPPORTS_CONSERVATIVE_RASTER device property to determine support.
    ///
    /// By default, conservative rasterization is disabled.
    ///
    ...
    /// snapped to fixed-point positions.  By default, the number of bits
    /// of fractional precision within a pixel is given by the device
    /// property SUBPIXEL_BITS.  This function enables applications to
    /// increase the subpixel precision for snapped vertex positions.  If
    /// conservative rasterization is disabled, additional bias bits have
    /// no effect on rasterization.
    ///
    ...
    /// the corresponding texture _dstTexture_.  This parameter identifies
    /// a specific mipmap level, set of layers or alternate format in the
    ...
    inline void CopyBufferToTexture(BufferAddress src,
                                    ...
                                    NVN_NOESCAPE const TextureView *dstView,
                                    NVN_NOESCAPE const CopyRegion *dstRegion,
                                    CopyFlags flags);
    ...
    /// the copy.
    /// A texture view, if specified, can be used to override properties of
    /// the corresponding texture _srcTexture_.  This parameter identifies
    /// a specific mipmap level, set of layers or alternate format in the
    /// specified texture.  Other fields, such as swizzle, are ignored.  If
    ...
    inline void CopyTextureToBuffer(const Texture *srcTexture,
                                    NVN_NOESCAPE const TextureView *srcView,
                                    NVN_NOESCAPE const CopyRegion *srcRegion,
                                    BufferAddress dst,
                                    CopyFlags flags);
    ...
    /// INVALIDATE_TEXTURE or by waiting on a Sync object.  When clearing a
    /// linear texture, the texture must have been created with the
    /// LINEAR_RENDER_TARGET flag set.
    ///
    /// \param dstTexture Texture object used as the destination for the
    /// clear.
    ///
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// CommandBuffer::UpdateUniformBuffer, but should be faster in common
    /// cases.  Not compatible with the NVN debug layer.
    ///
    /// \param buffer GPU address of the full uniform buffer to update.
    /// Must be a multiple of 256 (the value of
    ...
    /// \param data New data to write into the uniform buffer.  The data is
    /// consumed before the call returns.
    inline void UpdateUniformBuffer_fastpath(BufferAddress buffer,
                                             size_t bufferSize,
                                             ptrdiff_t updateOffset,
                                             size_t updateSize,
                                             NVN_NOESCAPE const void *data);
    ...
    /// eight bytes starting at _bufferAddress_ + 16.  Note that this
    /// layout is consistent with two counter query results stored
    ...
    inline void SetRenderTargets(int numColors,
                                 NVN_NOESCAPE const Texture * const *colors,
                                 NVN_NOESCAPE const TextureView * const *colorViews,
                                 const Texture *depthStencil,
                                 NVN_NOESCAPE const TextureView *depthStencilView);
    ...
    /// will become undefined.  This command can be useful for saving
    /// memory bandwidth for 'transient' rendering operations such as
    /// rendering to a multisample texture.  After rendering to a
    /// multisample texture, the contents are often filtered to produce
    /// single-sample texels using CommandBuffer::Downsample.  Discarding
    /// the contents of a multisample texture after the downsample allows
    /// the GPU to throw away its contents before writing values to memory.
    ///
    ...
    /// values will become undefined.  This command can be useful for
    /// saving memory bandwidth for 'transient' rendering operations such
    /// as rendering to a depth/stencil buffer that is only used to
    /// generate a color buffer image in the same frame.  Discarding the
    /// contents of a texture allows the GPU to throw away its contents
    /// before writing values to main memory.
    ///
    /// Only the portion of the render target contained inside the current
    /// scissor rectangle will be discarded.
    ...
    inline void DrawTexture(TextureHandle texture,
                            NVN_NOESCAPE const DrawTextureRegion *dstRegion,
                            NVN_NOESCAPE const DrawTextureRegion *srcRegion);
    ...
    inline void BindCoverageModulationTable(NVN_NOESCAPE const float *entries);
    ...
    /// \brief Specify new lossy color reduction thresholds for a format
    /// class.
    ///
    ...
    /// groups are pushed to the top of the debug group stack.  Debug
    /// groups are strictly hierarchical and their sequences may be nested
    /// within other debug groups but can not overlap.  These groups may
    /// then be used by the debug layer, an external debugger, or a
    /// profiler tool.
    ///
    ...
    ///
    /// \param domainId Debug domain ID.
    /// It is given by nvn::Device::GenerateDebugDomainId.  Different
    /// domain identifiers help debug tools to discriminate between
    /// concurrent, possibly overlapping, debug group stacks.
    ///
    ...
    /// nvn::CommandBuffer::PushDebugGroupStatic.  The contents of
    /// `description` are copied, and the copy is automatically stored as
    /// long as necessary.
    ///
    /// \param domainId Debug domain ID.
    /// It is given by nvn::Device::GenerateDebugDomainId.  Different
    ...
    /// stencil culling.  ZCull will only try to discard pixels/fragments
    /// if the cull criteria matches the current stencil state and if the
    ...
    /// \brief Override the default number of fragment shader warps in a
    /// single subtile.
    ///
    /// Fragments generated by rasterization are arranged into groups
    /// called _subtiles_, which are processed as a unit by the fragment
    /// shading hardware.  The subtile size is configured by the NVN
    /// driver, which computes a default subtile size based on properties
    /// of the fragment shader and GPU hardware.  Larger subtiles can
    /// result in better locality for memory accesses, but also have
    /// increased overhead when launching, synchronizing, and retiring the
    /// larger sets of fragment shaders.  This command allows applications
    /// to override the default subtile size in cases where the defaults
    /// are non-optimal.
    ///
    /// \param override Override the default subtile size.
    /// If true, use the subtile size specified by _subtileSize_.
    /// Otherwise, use a default subtile size derived from program state.
    ///
    /// \param subtileSize Subtile size used when overriding is enabled.
    /// When _override_ is true, this specifies a limit on the maximum
    /// number of fragment shader warps contained in a single subtile.
    /// This parameter is expressed as a fraction of the maximum possible
    /// subtile size, with 128 specifying the maximum size.
    inline void OverrideSubtileSize(NVNboolean override,
                                    int subtileSize);

    /// \brief Insert a checkpoint event marker to the command set.
    ///
    /// Inserts a checkpoint event marker into a command set.  In case of a
    /// GPU crash, the checkpoint event markers that were last processed by
    /// the GPU will be included in NVN Aftermath crash dumps to aid in
    /// narrowing down the command that has led to the crash.
    /// The lifetime of the marker data specified when calling this entry
    /// point is managed by the application.  The data is not copied and
    /// the application must ensure the memory pointed to by markerData is
    /// valid at least as long as the currently recorded command set can
    /// still be executed on a queue.
    /// If the checkpoint is set sucessfully, NVN_TRUE is returned.  If any
    /// of the passed in parameters is invalid or a problem occurs when
    /// setting the checkpoint, NVN_FALSE is returned and no checkpoint is
    /// set.
    ///
    /// \note This function is a no-op if the application is not linked
    /// with the NVN Aftermath target library.
    ///
    /// \param markerData Data to associate with the checkpoint event
    /// marker.
    ///
    /// \param markerDataSize Size (in bytes) of the checkpoint event
    /// marker data.
    inline NVNboolean InsertCheckpointStatic(const void *markerData,
    ...
    /// narrowing down the command that has led to the crash.
    /// The lifetime of the marker data specified when calling this entry
    /// point only needs to extend for the duration of the call.  A copy of
    /// the data is made and the application can free the passed in marker
    /// data buffer immediately after this function returns.
    /// If the checkpoint is set sucessfully, NVN_TRUE is returned.  If any
    /// of the passed in parameters is invalid or a problem occurs when
    /// setting the checkpoint, NVN_FALSE is returned and no checkpoint is
    /// set.
    ...
    /// nvn::DeviceInfo::CHECKPOINT_MAXIMUM_DYNAMIC_MARKER_DATA_SIZE).
    inline NVNboolean InsertCheckpointDynamic(NVN_NOESCAPE const void *markerData,
                                              size_t markerDataSize);

    /// \brief Enables/Disables the compression for ZF32 values in the
    /// ZCull Buffer.
    ///
    /// By default ZF32 values (i.e., the contents of nvn::Format::DEPTH32F
    /// or nvn::Format::DEPTH32F_STENCIL8 depth buffers) are compressed in
    /// the ZCull buffer, i.e.  equivalent to setting 'enable' to 'true'.
    /// Since the compression is lossy, this can significantly reduce the
    /// ZCull performance for depth values near 0.0.  When using GREATER as
    /// depth function it is recommended to disable ZF32Compression for the
    /// case where precision near 0.0 is required.
    /// Toggling the ZF32 compression mode is a comparatively slow
    /// operation.  Frequent toggles may adversely affect performance.
    /// After toggling, the result of any ZCull test will be undefined
    /// until after a depth buffer is cleared.
    ///
    /// \param enable If NVN_TRUE, enable ZF32 compression.  Otherwise
    /// disable it
    inline void SetZCullZF32CompressionEnable(NVNboolean enable);
...
public:
    ...
    /// \brief Specify the number of the color target to update via the
    /// BlendState object.  Fastpath variant.
    ///
    /// \note:  This is functionally equivalent to
    /// BlendState::SetBlendTarget, but should be faster in common cases.
    /// Not compatible with the NVN debug layer.
    ///
    /// \param target Color target number to update when binding the blend
    /// state object.
    inline BlendState & SetBlendTarget_fastpath(int target);
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// BlendState::SetBlendFunc, but should be faster in common cases.
    /// Not compatible with the NVN debug layer.
    ///
    /// \param srcFunc Blend factor used for source RGB color components.
    ///
    ...
    /// \param dstFuncAlpha Blend factor used for destination alpha color
    /// components.
    inline BlendState & SetBlendFunc_fastpath(BlendFunc srcFunc,
                                              BlendFunc dstFunc,
                                              BlendFunc srcFuncAlpha,
                                              BlendFunc dstFuncAlpha);
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// BlendState::SetBlendEquation, but should be faster in common cases.
    ///  Not compatible with the NVN debug layer.
    ///
    /// \param modeRGB Blend equation used for RGB color components.
    ///
    /// \param modeAlpha Blend equation used for alpha color components.
    inline BlendState & SetBlendEquation_fastpath(BlendEquation modeRGB,
                                                  BlendEquation modeAlpha);
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// BlendState::SetAdvancedMode, but should be faster in common cases.
    /// Not compatible with the NVN debug layer.
    ///
    /// \param overlap Mode to use for advanced blending equations.
    inline BlendState & SetAdvancedMode_fastpath(BlendAdvancedMode overlap);
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// BlendState::SetAdvancedOverlap, but should be faster in common
    /// cases.  Not compatible with the NVN debug layer.
    ///
    /// \param overlap Overlap mode used for advanced blending equations.
    inline BlendState & SetAdvancedOverlap_fastpath(BlendAdvancedOverlap overlap);
    ...
    /// \brief Specify whether the source color is considered premultiplied
    /// for advanced blending equations.  Fastpath variant.
    ///
    /// Advanced blending in NVN provides capabilities similar to those
    /// provided by the NV_blend_equation_advanced OpenGL extension.  By
    /// default, blending is performed with pre-multiplied colors, where a
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// BlendState::SetAdvancedPremultipliedSrc, but should be faster in
    /// common cases.  Not compatible with the NVN debug layer.
    ///
    /// \param premultiplied TRUE if source color components are considered
    /// to be pre-multiplied by the source alpha.
    inline BlendState & SetAdvancedPremultipliedSrc_fastpath(NVNboolean premultiplied);
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// BlendState::SetAdvancedNormalizedDst, but should be faster in
    /// common cases.  Not compatible with the NVN debug layer.
    ///
    /// \param normalized TRUE if destination color components are unsigned
    /// normalized ([0,1]) values.
    inline BlendState & SetAdvancedNormalizedDst_fastpath(NVNboolean normalized);
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// BlendState::GetBlendTarget, but should be faster in common cases.
    /// Not compatible with the NVN debug layer.
    inline int GetBlendTarget_fastpath() const;
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// BlendState::GetAdvancedPremultipliedSrc, but should be faster in
    /// common cases.  Not compatible with the NVN debug layer.
    inline NVNboolean GetAdvancedPremultipliedSrc_fastpath() const;
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// BlendState::GetAdvancedNormalizedDst, but should be faster in
    /// common cases.  Not compatible with the NVN debug layer.
    inline NVNboolean GetAdvancedNormalizedDst_fastpath() const;
...
public:
    ...
    /// operations for each color target.
    ///
    /// \note:  This is functionally equivalent to ColorState::SetLogicOp,
    /// but should be faster in common cases.  Not compatible with the NVN
    /// debug layer.
    ///
    /// \param logicOp Logical operation to perform on all color targets.
    inline ColorState & SetLogicOp_fastpath(LogicOp logicOp);
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// ColorState::SetAlphaTest, but should be faster in common cases.
    /// Not compatible with the NVN debug layer.
    ///
    /// \param alphaTest Comparison function used by the alpha test.
    inline ColorState & SetAlphaTest_fastpath(AlphaFunc alphaTest);
...
public:
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// DepthStencilState::SetDepthTestEnable, but should be faster in
    /// common cases.  Not compatible with the NVN debug layer.
    ///
    /// \param enable Enable (TRUE) or disable (FALSE) depth testing.
    inline DepthStencilState & SetDepthTestEnable_fastpath(NVNboolean enable);
    ...
    /// \brief Specify whether the depth buffer should be updated by the
    /// depth test.  Fastpath variant.
    ///
    /// If depth testing is disabled, the depth buffer is not updated,
    /// regardless of the depth write enable.
    ///
    /// \note:  This is functionally equivalent to
    /// DepthStencilState::SetDepthWriteEnable, but should be faster in
    /// common cases.  Not compatible with the NVN debug layer.
    ///
    /// \param enable Enable (TRUE) or disable (FALSE) writes to the depth
    /// buffer.
    inline DepthStencilState & SetDepthWriteEnable_fastpath(NVNboolean enable);
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// DepthStencilState::SetDepthFunc, but should be faster in common
    /// cases.  Not compatible with the NVN debug layer.
    ///
    /// \param func Comparison function used when depth testing is enabled.
    /// If depth testing is disabled, the depth function has no effect.
    inline DepthStencilState & SetDepthFunc_fastpath(DepthFunc func);
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// DepthStencilState::SetStencilTestEnable, but should be faster in
    /// common cases.  Not compatible with the NVN debug layer.
    ///
    /// \param enable Enable (TRUE) or disable (FALSE) stencil testing.
    inline DepthStencilState & SetStencilTestEnable_fastpath(NVNboolean enable);
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// DepthStencilState::GetDepthTestEnable, but should be faster in
    /// common cases.  Not compatible with the NVN debug layer.
    inline NVNboolean GetDepthTestEnable_fastpath() const;
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// DepthStencilState::GetDepthWriteEnable, but should be faster in
    /// common cases.  Not compatible with the NVN debug layer.
    inline NVNboolean GetDepthWriteEnable_fastpath() const;
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// DepthStencilState::GetStencilTestEnable, but should be faster in
    /// common cases.  Not compatible with the NVN debug layer.
    inline NVNboolean GetStencilTestEnable_fastpath() const;
...
public:
    ...
    /// \brief Enable or disable multisample rasterization.  Fastpath
    /// variant.
    ///
    /// \note:  This is functionally equivalent to
    /// MultisampleState::SetMultisampleEnable, but should be faster in
    /// common cases.  Not compatible with the NVN debug layer.
    ///
    /// \param enable Enable (TRUE) or disable (FALSE) multisample
    /// rasterization.
    ...
    /// multisample rasterization is disabled, a single coverage value is
    /// used for all samples in a pixel.
    inline MultisampleState & SetMultisampleEnable_fastpath(NVNboolean enable);
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// MultisampleState::SetSamples, but should be faster in common cases.
    ///  Not compatible with the NVN debug layer.
    ///
    /// \param samples Specifies the number of samples in the framebuffer.
    /// If non-multisample framebuffers are used, the value should be
    /// specified as 0.
    inline MultisampleState & SetSamples_fastpath(int samples);
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// MultisampleState::SetAlphaToCoverageEnable, but should be faster in
    /// common cases.  Not compatible with the NVN debug layer.
    ///
    /// \param enable Enable (TRUE) or disable (FALSE) the
    /// alpha-to-coverage operation.
    inline MultisampleState & SetAlphaToCoverageEnable_fastpath(NVNboolean enable);
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// MultisampleState::SetAlphaToCoverageDither, but should be faster in
    /// common cases.  Not compatible with the NVN debug layer.
    ///
    /// \param dither Enable (TRUE) or disable (FALSE) the
    /// alpha-to-coverage dithering operation.
    inline MultisampleState & SetAlphaToCoverageDither_fastpath(NVNboolean dither);

    /// \brief Query the multisample rasterization enable.
    inline NVNboolean GetMultisampleEnable() const;

    /// \brief Query the multisample rasterization enable.  Fastpath
    /// variant.
    ///
    /// \note:  This is functionally equivalent to
    /// MultisampleState::GetMultisampleEnable, but should be faster in
    /// common cases.  Not compatible with the NVN debug layer.
    inline NVNboolean GetMultisampleEnable_fastpath() const;
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// MultisampleState::GetSamples, but should be faster in common cases.
    ///  Not compatible with the NVN debug layer.
    inline int GetSamples_fastpath() const;
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// MultisampleState::GetAlphaToCoverageEnable, but should be faster in
    /// common cases.  Not compatible with the NVN debug layer.
    inline NVNboolean GetAlphaToCoverageEnable_fastpath() const;

    /// \brief Query the alpha-to-coverage dithering enable.
    inline NVNboolean GetAlphaToCoverageDither() const;

    /// \brief Query the alpha-to-coverage dithering enable.  Fastpath
    /// variant.
    ///
    /// \note:  This is functionally equivalent to
    /// MultisampleState::GetAlphaToCoverageDither, but should be faster in
    /// common cases.  Not compatible with the NVN debug layer.
    inline NVNboolean GetAlphaToCoverageDither_fastpath() const;
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// MultisampleState::SetRasterSamples, but should be faster in common
    /// cases.  Not compatible with the NVN debug layer.
    ///
    /// \param rasterSamples Raster sample count.
    /// If depth or stencil tests are performed with target-independent
    ...
    /// raster sample count must be greater than or equal to the color
    /// sample count set by MultisampleState::SetSamples.
    inline MultisampleState & SetRasterSamples_fastpath(int rasterSamples);

    /// \brief Query the raster sample count for target-independent
    /// rasterization.
    inline int GetRasterSamples() const;

    /// \brief Query the raster sample count for target-independent
    /// rasterization.  Fastpath variant.
    ///
    /// \note:  This is functionally equivalent to
    /// MultisampleState::GetRasterSamples, but should be faster in common
    /// cases.  Not compatible with the NVN debug layer.
    inline int GetRasterSamples_fastpath() const;
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// MultisampleState::SetCoverageModulationMode, but should be faster
    /// in common cases.  Not compatible with the NVN debug layer.
    ///
    /// \param mode Specifies the set of color channels to multiply by the
    /// coverage modulation factor.
    inline MultisampleState & SetCoverageModulationMode_fastpath(CoverageModulationMode mode);
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// MultisampleState::SetCoverageToColorEnable, but should be faster in
    /// common cases.  Not compatible with the NVN debug layer.
    ///
    /// \param enable Fragment coverage-to-color enable.
    inline MultisampleState & SetCoverageToColorEnable_fastpath(NVNboolean enable);
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// MultisampleState::GetCoverageToColorEnable, but should be faster in
    /// common cases.  Not compatible with the NVN debug layer.
    inline NVNboolean GetCoverageToColorEnable_fastpath() const;
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// MultisampleState::SetCoverageToColorOutput, but should be faster in
    /// common cases.  Not compatible with the NVN debug layer.
    ///
    /// \param color Color output number updated by the fragment
    /// coverage-to-color feature.
    inline MultisampleState & SetCoverageToColorOutput_fastpath(int color);
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// MultisampleState::GetCoverageToColorOutput, but should be faster in
    /// common cases.  Not compatible with the NVN debug layer.
    inline int GetCoverageToColorOutput_fastpath() const;
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// MultisampleState::SetSampleLocationsEnable, but should be faster in
    /// common cases.  Not compatible with the NVN debug layer.
    ///
    /// \param enable Enable/disable programmble sample locations.
    inline MultisampleState & SetSampleLocationsEnable_fastpath(NVNboolean enable);
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// MultisampleState::GetSampleLocationsEnable, but should be faster in
    /// common cases.  Not compatible with the NVN debug layer.
    inline NVNboolean GetSampleLocationsEnable_fastpath() const;
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// MultisampleState::SetSampleLocationsGridEnable, but should be
    /// faster in common cases.  Not compatible with the NVN debug layer.
    ///
    /// \param enable Enable or disable the programmble sample locations
    /// grid.
    ...
    /// pixel in a fixed-size grid.  If set to NVN_FALSE, the same
    /// locations are used for all pixels.
    inline MultisampleState & SetSampleLocationsGridEnable_fastpath(NVNboolean enable);
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// MultisampleState::GetSampleLocationsGridEnable, but should be
    /// faster in common cases.  Not compatible with the NVN debug layer.
    inline NVNboolean GetSampleLocationsGridEnable_fastpath() const;
    ...
    /// identifying the location of a sample within its pixel.  The
    /// location of the origin (0.0, 0.0) follows the window origin mode,
    /// which is set by nvn::Device::SetWindowOriginMode.
    ///
...
public:
    ...
    /// \brief Specify controls for culling of front- and back-facing
    /// polygons.  Fastpath variant.
    ///
    /// \note:  This is functionally equivalent to
    /// PolygonState::SetCullFace, but should be faster in common cases.
    /// Not compatible with the NVN debug layer.
    ///
    /// \param face Specifies the set of faces (if any) that should be
    /// cullled.
    inline PolygonState & SetCullFace_fastpath(Face face);
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// PolygonState::SetFrontFace, but should be faster in common cases.
    /// Not compatible with the NVN debug layer.
    ///
    /// \param face Treat clockwise (CW) or counter-clockwise (CCW)
    /// polygons as front-facing.
    inline PolygonState & SetFrontFace_fastpath(FrontFace face);
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// PolygonState::SetPolygonMode, but should be faster in common cases.
    ///  Not compatible with the NVN debug layer.
    ///
    /// \param polygonMode Render polygons as points (POINT), lines (LINE),
    /// or triangles (FILL).
    inline PolygonState & SetPolygonMode_fastpath(PolygonMode polygonMode);
...
public:
    ...
    /// vertex attribute.  Specifying format is mandatory, and doing so
    /// also enables the vertex attribute for fetching.
    ///
    ...
    /// nvn::DeviceInfo::VERTEX_ATTRIBUTE_STATE_MAXIMUM_RELATIVE_OFFSET).
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// VertexAttribState::SetStreamIndex, but should be faster in common
    /// cases.  Not compatible with the NVN debug layer.
    ///
    /// \param streamIndex Index of the stream to use for the attribute.
    /// Must be less than 16 (the value of
    /// nvn::DeviceInfo::VERTEX_BUFFER_BINDINGS).
    inline VertexAttribState & SetStreamIndex_fastpath(int streamIndex);
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// VertexAttribState::GetStreamIndex, but should be faster in common
    /// cases.  Not compatible with the NVN debug layer.
    inline int GetStreamIndex_fastpath() const;
...
public:
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// VertexStreamState::SetStride, but should be faster in common cases.
    ///  Not compatible with the NVN debug layer.
    ///
    /// \param stride Stride (in bytes) between vertex attribute records
    /// for the specified binding point.
    inline VertexStreamState & SetStride_fastpath(ptrdiff_t stride);
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// VertexStreamState::SetDivisor, but should be faster in common
    /// cases.  Not compatible with the NVN debug layer.
    ///
    /// \param divisor Frequency divisor for vertex attributes using the
    /// specified binding point.
    inline VertexStreamState & SetDivisor_fastpath(int divisor);
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// VertexStreamState::GetStride, but should be faster in common cases.
    ///  Not compatible with the NVN debug layer.
    inline ptrdiff_t GetStride_fastpath() const;
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// VertexStreamState::GetDivisor, but should be faster in common
    /// cases.  Not compatible with the NVN debug layer.
    inline int GetDivisor_fastpath() const;
...
public:
    ...
    /// shading hardware.  The subtile size is configured by the NVN
    /// driver, which computes a default subtile size based on properties
    /// of the fragment shader and GPU hardware.  This command returns the
    /// default subtile size for _program_, which can be overridden by
    /// CommandBuffer::OverrideSubtileSize.  Returns zero if _program_ does
    /// not include a fragment shader.
    inline int GetSubtileSize() const;
    ...
    /// not include a fragment shader.
    ///
    /// \note:  This is functionally equivalent to Program::GetSubtileSize,
    /// but should be faster in common cases.  Not compatible with the NVN
    /// debug layer.
    inline int GetSubtileSize_fastpath() const;
...
public:
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// MemoryPoolBuilder::GetDevice, but should be faster in common cases.
    ///  Not compatible with the NVN debug layer.
    inline const Device * GetDevice_fastpath() const;
    ...
    /// \brief Query the memory allocation associated with a
    /// MemoryPoolBuilder.  Fastpath variant.
    ///
    /// \note:  This is functionally equivalent to
    /// MemoryPoolBuilder::GetMemory, but should be faster in common cases.
    ///  Not compatible with the NVN debug layer.
    inline void * GetMemory_fastpath() const;
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// MemoryPoolBuilder::GetSize, but should be faster in common cases.
    /// Not compatible with the NVN debug layer.
    inline size_t GetSize_fastpath() const;
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// MemoryPoolBuilder::GetFlags, but should be faster in common cases.
    /// Not compatible with the NVN debug layer.
    inline MemoryPoolFlags GetFlags_fastpath() const;
...
public:
    ...
    /// video memory on the discrete GPU is exhausted.  Reducing the size
    /// of the memory pool and/or shutting down other applications that
    ...
    /// flush is performed with 64-byte granularity, matching the size of a
    /// CPU cache line.  The GPU cache line size is 128 bytes, so there may
    /// be interactions with nearby memory ranges that share a cache line
    /// with the flushed range.  If two non-coherent memory ranges are
    /// separately managed by the CPU and the GPU, it is recommended that
    /// they be allocated from distinct 128-byte aligned ranges.  See \ref
    ...
    /// \note On NX, calling nn::os::FlushDataCache on an equivalent memory
    /// range will have the same effect as calling this function.  The
    /// invalidate is performed with 64-byte granularity, matching the size
    /// of a CPU cache line.  The GPU cache line size is 128 bytes, so
    /// there may be interactions with nearby memory ranges that share a
    /// cache line with the flushed range.  If two non-coherent memory
    /// ranges are separately managed by the CPU and the GPU, it is
    /// recommended that they be allocated from distinct 128-byte aligned
    /// ranges.  See \ref nvn_guide_memory_pool_coherency in the NVN
...
public:
    ...
    /// descriptors are written.  Texture descriptors hold information
    /// about textures and are used by the GPU when using textures in
    /// rendering operations.  Texture descriptors are written into the
    /// pool using RegisterTexture.  The pool itself must be set to the
    /// queue before rendering with any individual textures.  A number of
    /// entries at the beginning of the pool are reserved for use by the
    /// driver.  The number of reserved entries can be found by querying
    /// the device property RESERVED_TEXTURE_DESCRIPTORS.  The size of each
    /// entry can be found by querying TEXTURE_DESCRIPTOR_SIZE.
    ///
    /// \param memoryPool MemoryPool object providing storage for the
    /// texture pool.  The memory pool must be CPU-accessible (except on
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// TexturePool::GetMemoryPool, but should be faster in common cases.
    /// Not compatible with the NVN debug layer.
    inline const MemoryPool * GetMemoryPool_fastpath() const;
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// TexturePool::GetMemoryOffset, but should be faster in common cases.
    ///  Not compatible with the NVN debug layer.
    inline ptrdiff_t GetMemoryOffset_fastpath() const;
    ...
    /// \brief Query the number of descriptors in the memory of a
    /// TexturePool object.  Fastpath variant.
    ///
    /// \note:  This is functionally equivalent to TexturePool::GetSize,
    /// but should be faster in common cases.  Not compatible with the NVN
    /// debug layer.
    inline int GetSize_fastpath() const;
...
public:
    ...
    /// descriptors are written.  Sampler descriptors hold information
    /// about textures and are used by the GPU when using samplers in
    /// rendering operations.  Sampler descriptors are written into the
    /// pool using the RegisterSampler or RegisterSamplerBuilder methods.
    /// The pool itself must be set to the queue before rendering with any
    /// individual textures.  A number of entries at the beginning of the
    /// pool are reserved for use by the driver.  The number of reserved
    /// entries can be found by querying the device property
    /// RESERVED_SAMPLER_DESCRIPTORS.  The size of each entry can be found
    /// by querying SAMPLER_DESCRIPTOR_SIZE.
    ///
    /// \param memoryPool MemoryPool object providing storage for the
    /// sampler pool.  The memory pool must be CPU-accessible (except on
    /// the Windows reference implementation).
    ///
    /// \param offset Offset (in bytes) of the first byte of memory pool
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// SamplerPool::GetMemoryPool, but should be faster in common cases.
    /// Not compatible with the NVN debug layer.
    inline const MemoryPool * GetMemoryPool_fastpath() const;
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// SamplerPool::GetMemoryOffset, but should be faster in common cases.
    ///  Not compatible with the NVN debug layer.
    inline ptrdiff_t GetMemoryOffset_fastpath() const;
    ...
    /// \brief Query the number of descriptors in the memory of a
    /// SamplerPool object.  Fastpath variant.
    ///
    /// \note:  This is functionally equivalent to SamplerPool::GetSize,
    /// but should be faster in common cases.  Not compatible with the NVN
    /// debug layer.
    inline int GetSize_fastpath() const;
...
public:
    ...
    /// \brief Set the range of a MemoryPool used for a Buffer object
    /// created from the BufferBuilder.  Fastpath variant.
    ///
    /// \note:  This is functionally equivalent to
    /// BufferBuilder::SetStorage, but should be faster in common cases.
    /// Not compatible with the NVN debug layer.
    ///
    /// \param pool MemoryPool object providing storage for the new Buffer
    /// object.
    ...
    ///
    /// \param size Size (in bytes) of the new buffer object.
    inline BufferBuilder & SetStorage_fastpath(MemoryPool *pool,
                                               ptrdiff_t offset,
                                               size_t size);
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// BufferBuilder::GetDevice, but should be faster in common cases.
    /// Not compatible with the NVN debug layer.
    inline const Device * GetDevice_fastpath() const;
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// BufferBuilder::GetMemoryPool, but should be faster in common cases.
    ///  Not compatible with the NVN debug layer.
    inline MemoryPool * GetMemoryPool_fastpath() const;
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// BufferBuilder::GetMemoryOffset, but should be faster in common
    /// cases.  Not compatible with the NVN debug layer.
    inline ptrdiff_t GetMemoryOffset_fastpath() const;
    ...
    /// object.  Fastpath variant.
    ///
    /// \note:  This is functionally equivalent to BufferBuilder::GetSize,
    /// but should be faster in common cases.  Not compatible with the NVN
    /// debug layer.
    inline size_t GetSize_fastpath() const;
...
public:
    ...
    ///
    /// Annotates a Buffer object with a descriptive text label.  This
    /// label may then be used by the debug layer, an external debugger, or
    /// a profiler tool.
    ///
    ...
    /// \brief Query the memory pool used by a Buffer object.  Fastpath
    /// variant.
    ///
    /// \note:  This is functionally equivalent to Buffer::GetMemoryPool,
    /// but should be faster in common cases.  Not compatible with the NVN
    /// debug layer.
    inline MemoryPool * GetMemoryPool_fastpath() const;
    ...
    /// Fastpath variant.
    ///
    /// \note:  This is functionally equivalent to Buffer::GetMemoryOffset,
    /// but should be faster in common cases.  Not compatible with the NVN
    /// debug layer.
    inline ptrdiff_t GetMemoryOffset_fastpath() const;
    ...
    ///  Fastpath variant.
    ///
    /// \note:  This is functionally equivalent to Buffer::GetSize, but
    /// should be faster in common cases.  Not compatible with the NVN
    /// debug layer.
    inline size_t GetSize_fastpath() const;
...
public:
    ...
    /// ZCull save/restore buffer for a depth texture.  The buffer should
    /// be aligned to DeviceInfo::ZCULL_SAVE_RESTORE_ALIGNMENT bytes.
    ///
    ...
    /// MemoryPoolFlags::CPU_UNCACHED.  The flush is performed with 64-byte
    /// granularity, matching the size of a CPU cache line.  The GPU cache
    /// line size is 128 bytes, so there may be interactions with nearby
    /// memory ranges that share a cache line with the flushed range.  If
    /// two non-coherent memory ranges are separately managed by the CPU
    /// and the GPU, it is recommended that they be allocated from distinct
    /// 128-byte aligned ranges.  See \ref nvn_guide_memory_pool_coherency
    /// in the NVN Programming Guide for details and recommendations.
    ///
    ...
    /// pools created using MemoryPoolFlags::CPU_UNCACHED.  The invalidate
    /// is performed with 64-byte granularity, matching the size of a CPU
    /// cache line.  The GPU cache line size is 128 bytes, so there may be
    /// interactions with nearby memory ranges that share a cache line with
    /// the flushed range.  If two non-coherent memory ranges are
    /// separately managed by the CPU and the GPU, it is recommended that
    /// they be allocated from distinct 128-byte aligned ranges.  See \ref
    /// nvn_guide_memory_pool_coherency in the NVN Programming Guide for
    /// details and recommendations.
    ...
    /// \brief Returns the debug layer unique ID of this texture.  Returns
    /// 0 if debug layer disabled.
    inline uint64_t GetDebugID() const;
...
public:
    ...
    /// \brief Set the target (texture type) for Texture objects created
    /// from the TextureBuilder.  Fastpath variant.
    ///
    /// \note:  This is functionally equivalent to
    /// TextureBuilder::SetTarget, but should be faster in common cases.
    /// Not compatible with the NVN debug layer.
    ///
    /// \param target Texture target to use.
    inline TextureBuilder & SetTarget_fastpath(TextureTarget target);
    ...
    /// \brief Set the width (in texels) for Texture objects created from
    /// the TextureBuilder.  Fastpath variant.
    ///
    /// \note:  This is functionally equivalent to
    /// TextureBuilder::SetWidth, but should be faster in common cases.
    /// Not compatible with the NVN debug layer.
    ///
    /// \param width Width (in texels) used for new textures.
    inline TextureBuilder & SetWidth_fastpath(int width);
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// TextureBuilder::SetHeight, but should be faster in common cases.
    /// Not compatible with the NVN debug layer.
    ///
    /// \param height Height (in texels) used for new textures.
    inline TextureBuilder & SetHeight_fastpath(int height);
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// TextureBuilder::SetDepth, but should be faster in common cases.
    /// Not compatible with the NVN debug layer.
    ///
    /// \param depth Depth (in texels) used for new textures.
    inline TextureBuilder & SetDepth_fastpath(int depth);
    ...
    /// \brief Set the width, height, and depth (in texels) for Texture
    /// objects created from the TextureBuilder.  Fastpath variant.
    ///
    /// \note:  This is functionally equivalent to
    /// TextureBuilder::SetSize3D, but should be faster in common cases.
    /// Not compatible with the NVN debug layer.
    ///
    /// \param width Width (in texels) used for new textures.
    /// Must be greater than zero.
    ...
    /// \param depth Depth (in texels) used for new textures.
    /// Must be greater than zero.
    inline TextureBuilder & SetSize3D_fastpath(int width,
                                               int height,
                                               int depth);
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// TextureBuilder::SetLevels, but should be faster in common cases.
    /// Not compatible with the NVN debug layer.
    ///
    /// \param numLevels Number of mipmap levels to create for the texture.
    /// \li Must be greater than zero.
    ...
    /// Must be less than or equal to 15 (the value of
    /// nvn::DeviceInfo::MAX_TEXTURE_LEVELS).
    inline TextureBuilder & SetLevels_fastpath(int numLevels);
    ...
    /// \brief Set the Format for Texture objects created from the
    /// TextureBuilder.  Fastpath variant.
    ///
    /// Note that compressed formats are not supported for 1D or 1D array
    /// textures.
    ///
    /// \note:  This is functionally equivalent to
    /// TextureBuilder::SetFormat, but should be faster in common cases.
    /// Not compatible with the NVN debug layer.
    ///
    /// \param format Format to use for new textures.
    inline TextureBuilder & SetFormat_fastpath(Format format);
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// TextureBuilder::SetSamples, but should be faster in common cases.
    /// Not compatible with the NVN debug layer.
    ///
    /// \param samples Sample count to use for new textures.
    /// \li Must be zero for non-multisample textures (those with targets
    ...
    /// \li Must be 2, 4, 8 or 16 for multisample textures.
    /// \li May be 16 only for textures with a format of Format::STENCIL8.
    inline TextureBuilder & SetSamples_fastpath(int samples);
    ...
    /// \brief Set the values returned for depth/stencil Texture objects
    /// created from the TextureBuilder.  Fastpath variant.
    ///
    /// If the texture has a format with no depth or stencil components,
    /// the depth/stencil mode is ignored.  If depth/stencil mode selects
    /// the stencil component in a texture with only a depth component or
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// TextureBuilder::SetDepthStencilMode, but should be faster in common
    /// cases.  Not compatible with the NVN debug layer.
    ///
    /// \param mode Specifies whether to return depth or stencil values
    /// from a texture with depth and/or stencil components.
    inline TextureBuilder & SetDepthStencilMode_fastpath(TextureDepthStencilMode mode);

    /// \brief Returns the amount of memory (in bytes) required to hold a
    /// texture to be created using the provided TextureBuilder object.
    ...
    /// \brief Returns the offset (in bytes) of the first byte of storage
    /// used by a view of a Texture object created by the provided
    /// TextureBuilder.
    ///
    /// The offset returned is a value relative to the offset of the first
    /// byte of storage used by the full texture.  Textures are stored in
    ...
    /// nvn::Texture::GetViewOffset called on a Texture initialized from
    /// the provided TextureBuilder.
    ///
    /// \param textureView View of the texture used to determine an offset.
    inline ptrdiff_t GetViewOffset(const TextureView *textureView) const;
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// TextureBuilder::SetStorage, but should be faster in common cases.
    /// Not compatible with the NVN debug layer.
    ///
    /// \param pool MemoryPool object providing storage for the new Texture
    /// object.
    ...
    /// \li The provided offset must be a multiple of the required
    /// alignment for the provided TextureBuilder object.
    inline TextureBuilder & SetStorage_fastpath(MemoryPool *pool,
                                                ptrdiff_t offset);
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// TextureBuilder::GetDevice, but should be faster in common cases.
    /// Not compatible with the NVN debug layer.
    inline const Device * GetDevice_fastpath() const;
    ...
    /// \brief Query the flags for a TextureBuilder object.  Fastpath
    /// variant.
    ///
    /// \note:  This is functionally equivalent to
    /// TextureBuilder::GetFlags, but should be faster in common cases.
    /// Not compatible with the NVN debug layer.
    inline TextureFlags GetFlags_fastpath() const;

    /// \brief Query the target for a TextureBuilder object.
    inline TextureTarget GetTarget() const;

    /// \brief Query the target for a TextureBuilder object.  Fastpath
    /// variant.
    ///
    /// \note:  This is functionally equivalent to
    /// TextureBuilder::GetTarget, but should be faster in common cases.
    /// Not compatible with the NVN debug layer.
    inline TextureTarget GetTarget_fastpath() const;

    /// \brief Query the width for a TextureBuilder object.
    inline int GetWidth() const;

    /// \brief Query the width for a TextureBuilder object.  Fastpath
    /// variant.
    ///
    /// \note:  This is functionally equivalent to
    /// TextureBuilder::GetWidth, but should be faster in common cases.
    /// Not compatible with the NVN debug layer.
    inline int GetWidth_fastpath() const;

    /// \brief Query the height for a TextureBuilder object.
    inline int GetHeight() const;

    /// \brief Query the height for a TextureBuilder object.  Fastpath
    /// variant.
    ///
    /// \note:  This is functionally equivalent to
    /// TextureBuilder::GetHeight, but should be faster in common cases.
    /// Not compatible with the NVN debug layer.
    inline int GetHeight_fastpath() const;

    /// \brief Query the depth for a TextureBuilder object.
    inline int GetDepth() const;

    /// \brief Query the depth for a TextureBuilder object.  Fastpath
    /// variant.
    ///
    /// \note:  This is functionally equivalent to
    /// TextureBuilder::GetDepth, but should be faster in common cases.
    /// Not compatible with the NVN debug layer.
    inline int GetDepth_fastpath() const;

    /// \brief Query the level count for a TextureBuilder object.
    inline int GetLevels() const;

    /// \brief Query the level count for a TextureBuilder object.  Fastpath
    /// variant.
    ///
    /// \note:  This is functionally equivalent to
    /// TextureBuilder::GetLevels, but should be faster in common cases.
    /// Not compatible with the NVN debug layer.
    inline int GetLevels_fastpath() const;

    /// \brief Query the format for a TextureBuilder object.
    inline Format GetFormat() const;

    /// \brief Query the format for a TextureBuilder object.  Fastpath
    /// variant.
    ///
    /// \note:  This is functionally equivalent to
    /// TextureBuilder::GetFormat, but should be faster in common cases.
    /// Not compatible with the NVN debug layer.
    inline Format GetFormat_fastpath() const;
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// TextureBuilder::GetSamples, but should be faster in common cases.
    /// Not compatible with the NVN debug layer.
    inline int GetSamples_fastpath() const;
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// TextureBuilder::GetDepthStencilMode, but should be faster in common
    /// cases.  Not compatible with the NVN debug layer.
    inline TextureDepthStencilMode GetDepthStencilMode_fastpath() const;
    ...
    /// \brief Query the stride for a TextureBuilder object.  Fastpath
    /// variant.
    ///
    /// \note:  This is functionally equivalent to
    /// TextureBuilder::GetStride, but should be faster in common cases.
    /// Not compatible with the NVN debug layer.
    inline ptrdiff_t GetStride_fastpath() const;
    ...
    /// \brief Returns the name of the OpenGL or OpenGL ES texture to use
    /// as an external texture source.  Fastpath variant.
    ///
    /// If the returned name is zero, no source texture will be used.  See
    /// SetGLTextureName for details.
    ///
    /// \note:  This is functionally equivalent to
    /// TextureBuilder::GetGLTextureName, but should be faster in common
    /// cases.  Not compatible with the NVN debug layer.
    inline uint32_t GetGLTextureName_fastpath() const;
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// TextureBuilder::GetMemoryPool, but should be faster in common
    /// cases.  Not compatible with the NVN debug layer.
    inline MemoryPool * GetMemoryPool_fastpath() const;
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// TextureBuilder::GetMemoryOffset, but should be faster in common
    /// cases.  Not compatible with the NVN debug layer.
    inline ptrdiff_t GetMemoryOffset_fastpath() const;
...
public:
    ...
    /// \brief Set the minimum and maximum level of detail used for Sampler
    /// objects created from the SamplerBuilder.  Fastpath variant.
    ///
    /// When a sampler builder is used to initialize a sampler object,
    /// _min_ will be clamped to [0.0, 15.0], and _max_ will be clamped to
    /// [max(0.0, _min_), 15.0].
    ///
    /// \note:  This is functionally equivalent to
    /// SamplerBuilder::SetLodClamp, but should be faster in common cases.
    /// Not compatible with the NVN debug layer.
    ///
    /// \param min Specifies the minimum level of detail used with the
    /// sampler.
    ...
    /// sampler.
    /// \li Must be greater than or equal to min.
    inline SamplerBuilder & SetLodClamp_fastpath(float min,
                                                 float max);
    ...
    /// \brief Set the level-of-detail bias used for Sampler objects
    /// created from the SamplerBuilder.  Fastpath variant.
    ///
    /// When a sampler builder is used to initialize a sampler object,
    /// _bias_ will be clamped to [-16.0, 15.996].
    ///
    /// \note:  This is functionally equivalent to
    /// SamplerBuilder::SetLodBias, but should be faster in common cases.
    /// Not compatible with the NVN debug layer.
    ///
    /// \param bias Specifies the bias to add to the computed level of
    /// detail when the sampler is used.
    inline SamplerBuilder & SetLodBias_fastpath(float bias);
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// SamplerBuilder::SetMaxAnisotropy, but should be faster in common
    /// cases.  Not compatible with the NVN debug layer.
    ///
    /// \param maxAniso Specifies the maximum level of anisotropy used with
    /// the sampler.
    ...
    /// Must be less than or equal to 16 (the value of
    /// nvn::DeviceInfo::MAX_TEXTURE_ANISOTROPY).
    inline SamplerBuilder & SetMaxAnisotropy_fastpath(float maxAniso);
    ...
    /// \brief Set LOD snap used for Sampler objects created from the
    /// SamplerBuilder.  Fastpath variant.
    ///
    /// The texture LOD snap feature is an optimization that reduces
    /// texture bandwidth consumption with minification filters that
    /// average samples in multiple texture LODs.  When performing texture
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// SamplerBuilder::SetLodSnap, but should be faster in common cases.
    /// Not compatible with the NVN debug layer.
    ///
    /// \param lodSnap Specifies the LOD snap used for the sampler.
    /// Specifies the LOD snap used for the sampler.  Valid values are in
    ...
    /// are supported by the hardware; the driver will select a supported
    /// snap value close to the value provided in the sampler.
    inline SamplerBuilder & SetLodSnap_fastpath(float lodSnap);
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// SamplerBuilder::GetDevice, but should be faster in common cases.
    /// Not compatible with the NVN debug layer.
    inline const Device * GetDevice_fastpath() const;
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// SamplerBuilder::GetLodBias, but should be faster in common cases.
    /// Not compatible with the NVN debug layer.
    inline float GetLodBias_fastpath() const;
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// SamplerBuilder::GetMaxAnisotropy, but should be faster in common
    /// cases.  Not compatible with the NVN debug layer.
    inline float GetMaxAnisotropy_fastpath() const;
    ...
    /// \brief Query the LOD snap for a SamplerBuilder object.  Fastpath
    /// variant.
    ///
    /// \note:  This is functionally equivalent to
    /// SamplerBuilder::GetLodSnap, but should be faster in common cases.
    /// Not compatible with the NVN debug layer.
    inline float GetLodSnap_fastpath() const;
...
public:
    ...
    /// \brief Returns the debug layer unique ID of this sampler.  Returns
    /// 0 if debug layer disabled.
    inline uint64_t GetDebugID() const;
...
public:
    ...
    ///
    /// Annotates a Window object with a descriptive text label.  This
    /// label may then be used by the debug layer, an external debugger, or
    /// a profiler tool.
    ///
    ...
    /// \brief Query the native window for a Window object.  Fastpath
    /// variant.
    ///
    /// \note:  This is functionally equivalent to Window::GetNativeWindow,
    /// but should be faster in common cases.  Not compatible with the NVN
    /// debug layer.
    inline NVNnativeWindow GetNativeWindow_fastpath() const;

    /// \brief Query the present interval for a Window object.
    inline int GetPresentInterval() const;

    /// \brief Query the present interval for a Window object.  Fastpath
    /// variant.
    ///
    /// \note:  This is functionally equivalent to
    /// Window::GetPresentInterval, but should be faster in common cases.
    /// Not compatible with the NVN debug layer.
    inline int GetPresentInterval_fastpath() const;
    ...
    /// than _n_ refreshes since the previous update.  This limits the
    /// frame rate to 1 / _n_ of the display's refresh rate.
    ///
    /// \note The production environment will not support a present
    /// interval of 0.  In that environment, setting a present interval of
    /// 0 will behave as though 1 were specified.
    ///
    ...
    /// the texture rectangle gets scaled to fit the window size.  Passing
    /// a width or height of zero will reset the crop rectangle to match
    /// the dimension of the window texture.
    ///
    ...
    /// \brief Query the maximum available number of textures for
    /// presenting to this window.  Fastpath variant.
    ///
    /// This value is the same number of preallocated textures by
    /// nvn::WindowBuilder::SetTextures
    ///
    /// \note:  This is functionally equivalent to Window::GetNumTextures,
    /// but should be faster in common cases.  Not compatible with the NVN
    /// debug layer.
    inline int GetNumTextures_fastpath() const;
...
public:
    ...
    /// \brief Set the native window where the textures of this window will
    /// be presented to.  Fastpath variant.
    ///
    /// On NX, the native window should be of type
    /// nn::vi::NativeWindowHandle.
    ///
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// WindowBuilder::SetNativeWindow, but should be faster in common
    /// cases.  Not compatible with the NVN debug layer.
    ///
    /// \param nativeWindow NativeWindow where the textures of this window
    /// will be presented to.
    inline WindowBuilder & SetNativeWindow_fastpath(NVNnativeWindow nativeWindow);
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// WindowBuilder::SetTextures, but should be faster in common cases.
    /// Not compatible with the NVN debug layer.
    ///
    /// \param numTextures Number of color textures to use.
    /// Must be greater than or equal to 2 (the value of
    ...
    /// The pointer must remain valid until the window is initialized with
    /// Window::Initialize.
    inline WindowBuilder & SetTextures_fastpath(int numTextures,
                                                Texture * const *textures);
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// WindowBuilder::SetNumActiveTextures, but should be faster in common
    /// cases.  Not compatible with the NVN debug layer.
    ///
    /// \param numActiveTextures Number of color textures to use.  Must be
    /// less than or equal the number of textures set by
    ...
    /// Must be less than or equal to 4 (the value of
    /// nvn::DeviceInfo::MAX_TEXTURES_PER_WINDOW).
    inline WindowBuilder & SetNumActiveTextures_fastpath(int numActiveTextures);
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// WindowBuilder::GetDevice, but should be faster in common cases.
    /// Not compatible with the NVN debug layer.
    inline const Device * GetDevice_fastpath() const;
    ...
    /// \brief Query the number of textures associated with this
    /// WindowBuilder object.  Fastpath variant.
    ///
    /// \note:  This is functionally equivalent to
    /// WindowBuilder::GetNumTextures, but should be faster in common
    /// cases.  Not compatible with the NVN debug layer.
    inline int GetNumTextures_fastpath() const;
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// WindowBuilder::GetNativeWindow, but should be faster in common
    /// cases.  Not compatible with the NVN debug layer.
    inline NVNnativeWindow GetNativeWindow_fastpath() const;
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// WindowBuilder::GetNumActiveTextures, but should be faster in common
    /// cases.  Not compatible with the NVN debug layer.
    inline int GetNumActiveTextures_fastpath() const;
...
public:
    ...
    /// \brief Get the memory pool associated with this EventBuilder
    /// object.
    inline const MemoryPool * GetMemoryPool() const;
    ...
    ///
    /// \note:  This is functionally equivalent to
    /// EventBuilder::GetMemoryPool, but should be faster in common cases.
    /// Not compatible with the NVN debug layer.
    inline const MemoryPool * GetMemoryPool_fastpath() const;

    /// \brief Get the memory pool offset associated with this EventBuilder
    /// object.
    inline int64_t GetMemoryOffset() const;

    /// \brief Get the memory pool offset associated with this EventBuilder
    /// object.  Fastpath variant.
    ///
    /// \note:  This is functionally equivalent to
    /// EventBuilder::GetMemoryOffset, but should be faster in common
    /// cases.  Not compatible with the NVN debug layer.
    inline int64_t GetMemoryOffset_fastpath() const;
...
public:
    ...
    /// \brief Get the memory pool associated with this Event object.
    inline const MemoryPool * GetMemoryPool() const;

    /// \brief Get the memory pool associated with this Event object.
    /// Fastpath variant.
    ///
    /// \note:  This is functionally equivalent to Event::GetMemoryPool,
    /// but should be faster in common cases.  Not compatible with the NVN
    /// debug layer.
    inline const MemoryPool * GetMemoryPool_fastpath() const;

    /// \brief Get the memory pool offset associated with this Event
    /// object.
    inline int64_t GetMemoryOffset() const;
    ...
    /// object.  Fastpath variant.
    ///
    /// \note:  This is functionally equivalent to Event::GetMemoryOffset,
    /// but should be faster in common cases.  Not compatible with the NVN
    /// debug layer.
    inline int64_t GetMemoryOffset_fastpath() const;
...
typedef int (NVNAPIENTRY *ProgramGetSubtileSizeFunc) (const Program *program);
...
typedef ptrdiff_t (NVNAPIENTRY *TextureBuilderGetViewOffsetFunc) (const TextureBuilder *builder, const TextureView *textureView);
...
typedef int (NVNAPIENTRY *MultisampleStateGetRasterSamplesFunc) (const MultisampleState *multisample);
...
typedef void (NVNAPIENTRY *CommandBufferCallCommandsFunc) (CommandBuffer *cmdBuf, int numCommands, NVN_NOESCAPE const CommandHandle *handles);
typedef void (NVNAPIENTRY *CommandBufferCopyCommandsFunc) (CommandBuffer *cmdBuf, int numCommands, NVN_NOESCAPE const CommandHandle *handles);
typedef void (NVNAPIENTRY *CommandBufferBindBlendStateFunc) (CommandBuffer *cmdBuf, NVN_NOESCAPE const BlendState *blend);
typedef void (NVNAPIENTRY *CommandBufferBindChannelMaskStateFunc) (CommandBuffer *cmdBuf, NVN_NOESCAPE const ChannelMaskState *channelMask);
typedef void (NVNAPIENTRY *CommandBufferBindColorStateFunc) (CommandBuffer *cmdBuf, NVN_NOESCAPE const ColorState *color);
typedef void (NVNAPIENTRY *CommandBufferBindMultisampleStateFunc) (CommandBuffer *cmdBuf, NVN_NOESCAPE const MultisampleState *multisample);
typedef void (NVNAPIENTRY *CommandBufferBindPolygonStateFunc) (CommandBuffer *cmdBuf, NVN_NOESCAPE const PolygonState *polygon);
typedef void (NVNAPIENTRY *CommandBufferBindDepthStencilStateFunc) (CommandBuffer *cmdBuf, NVN_NOESCAPE const DepthStencilState *depthStencil);
typedef void (NVNAPIENTRY *CommandBufferBindVertexAttribStateFunc) (CommandBuffer *cmdBuf, int numAttribs, NVN_NOESCAPE const VertexAttribState *attribs);
typedef void (NVNAPIENTRY *CommandBufferBindVertexStreamStateFunc) (CommandBuffer *cmdBuf, int numStreams, NVN_NOESCAPE const VertexStreamState *streams);
...
typedef void (NVNAPIENTRY *CommandBufferBindVertexBuffersFunc) (CommandBuffer *cmdBuf, int first, int count, NVN_NOESCAPE const BufferRange *buffers);
...
typedef void (NVNAPIENTRY *CommandBufferBindUniformBuffersFunc) (CommandBuffer *cmdBuf, ShaderStage::Enum stage, int first, int count, NVN_NOESCAPE const BufferRange *buffers);
typedef void (NVNAPIENTRY *CommandBufferBindUniformBuffersDeferredFunc) (CommandBuffer *cmdBuf, ShaderStage::Enum stage, int first, int count, const BufferRange *buffers);
...
typedef void (NVNAPIENTRY *CommandBufferBindTransformFeedbackBuffersFunc) (CommandBuffer *cmdBuf, int first, int count, NVN_NOESCAPE const BufferRange *buffers);
...
typedef void (NVNAPIENTRY *CommandBufferBindStorageBuffersFunc) (CommandBuffer *cmdBuf, ShaderStage::Enum stage, int first, int count, NVN_NOESCAPE const BufferRange *buffers);
typedef void (NVNAPIENTRY *CommandBufferBindStorageBuffersDeferredFunc) (CommandBuffer *cmdBuf, ShaderStage::Enum stage, int first, int count, const BufferRange *buffers);
...
typedef void (NVNAPIENTRY *CommandBufferBindTexturesFunc) (CommandBuffer *cmdBuf, ShaderStage::Enum stage, int first, int count, NVN_NOESCAPE const TextureHandle *textures);
typedef void (NVNAPIENTRY *CommandBufferBindTexturesDeferredFunc) (CommandBuffer *cmdBuf, ShaderStage::Enum stage, int first, int count, const TextureHandle *textures);
...
typedef void (NVNAPIENTRY *CommandBufferBindImagesFunc) (CommandBuffer *cmdBuf, ShaderStage::Enum stage, int first, int count, NVN_NOESCAPE const ImageHandle *images);
typedef void (NVNAPIENTRY *CommandBufferBindImagesDeferredFunc) (CommandBuffer *cmdBuf, ShaderStage::Enum stage, int first, int count, const ImageHandle *images);
typedef void (NVNAPIENTRY *CommandBufferSetPatchSizeFunc) (CommandBuffer *cmdBuf, int size);
typedef void (NVNAPIENTRY *CommandBufferSetInnerTessellationLevelsFunc) (CommandBuffer *cmdBuf, NVN_NOESCAPE const float *levels);
typedef void (NVNAPIENTRY *CommandBufferSetOuterTessellationLevelsFunc) (CommandBuffer *cmdBuf, NVN_NOESCAPE const float *levels);
...
typedef void (NVNAPIENTRY *CommandBufferClearColorFunc) (CommandBuffer *cmdBuf, int index, NVN_NOESCAPE const float *color, int mask);
typedef void (NVNAPIENTRY *CommandBufferClearColoriFunc) (CommandBuffer *cmdBuf, int index, NVN_NOESCAPE const int *color, int mask);
typedef void (NVNAPIENTRY *CommandBufferClearColoruiFunc) (CommandBuffer *cmdBuf, int index, NVN_NOESCAPE const uint32_t *color, int mask);
...
typedef void (NVNAPIENTRY *CommandBufferSetViewportsFunc) (CommandBuffer *cmdBuf, int first, int count, NVN_NOESCAPE const float *ranges);
typedef void (NVNAPIENTRY *CommandBufferSetViewportSwizzlesFunc) (CommandBuffer *cmdBuf, int first, int count, NVN_NOESCAPE const ViewportSwizzle *swizzles);
typedef void (NVNAPIENTRY *CommandBufferSetScissorFunc) (CommandBuffer *cmdBuf, int x, int y, int w, int h);
typedef void (NVNAPIENTRY *CommandBufferSetScissorsFunc) (CommandBuffer *cmdBuf, int first, int count, NVN_NOESCAPE const int *rects);
...
typedef void (NVNAPIENTRY *CommandBufferSetDepthRangesFunc) (CommandBuffer *cmdBuf, int first, int count, NVN_NOESCAPE const float *ranges);
...
typedef void (NVNAPIENTRY *CommandBufferBindSeparateTexturesFunc) (CommandBuffer *cmdBuf, ShaderStage::Enum stage, int first, int count, NVN_NOESCAPE const SeparateTextureHandle *textureHandles);
typedef void (NVNAPIENTRY *CommandBufferBindSeparateSamplersFunc) (CommandBuffer *cmdBuf, ShaderStage::Enum stage, int first, int count, NVN_NOESCAPE const SeparateSamplerHandle *samplerHandles);
typedef void (NVNAPIENTRY *CommandBufferBindSeparateTexturesDeferredFunc) (CommandBuffer *cmdBuf, ShaderStage::Enum stage, int first, int count, const SeparateTextureHandle *textureHandles);
typedef void (NVNAPIENTRY *CommandBufferBindSeparateSamplersDeferredFunc) (CommandBuffer *cmdBuf, ShaderStage::Enum stage, int first, int count, const SeparateSamplerHandle *samplerHandles);
...
typedef void (NVNAPIENTRY *CommandBufferSetBlendColorFunc) (CommandBuffer *cmdBuf, NVN_NOESCAPE const float *blendColor);
...
typedef void (NVNAPIENTRY *CommandBufferCopyBufferToTextureFunc) (CommandBuffer *cmdBuf, BufferAddress src, const Texture *dstTexture, NVN_NOESCAPE const TextureView *dstView, NVN_NOESCAPE const CopyRegion *dstRegion, int flags);
typedef void (NVNAPIENTRY *CommandBufferCopyTextureToBufferFunc) (CommandBuffer *cmdBuf, const Texture *srcTexture, NVN_NOESCAPE const TextureView *srcView, NVN_NOESCAPE const CopyRegion *srcRegion, BufferAddress dst, int flags);
typedef void (NVNAPIENTRY *CommandBufferCopyTextureToTextureFunc) (CommandBuffer *cmdBuf, const Texture *srcTexture, NVN_NOESCAPE const TextureView *srcView, NVN_NOESCAPE const CopyRegion *srcRegion, const Texture *dstTexture, NVN_NOESCAPE const TextureView *dstView, NVN_NOESCAPE const CopyRegion *dstRegion, int flags);
...
typedef void (NVNAPIENTRY *CommandBufferClearTextureFunc) (CommandBuffer *cmdBuf, const Texture *dstTexture, NVN_NOESCAPE const TextureView *dstView, NVN_NOESCAPE const CopyRegion *dstRegion, NVN_NOESCAPE const float *color, int mask);
typedef void (NVNAPIENTRY *CommandBufferClearTextureiFunc) (CommandBuffer *cmdBuf, const Texture *dstTexture, NVN_NOESCAPE const TextureView *dstView, NVN_NOESCAPE const CopyRegion *dstRegion, NVN_NOESCAPE const int *color, int mask);
typedef void (NVNAPIENTRY *CommandBufferClearTextureuiFunc) (CommandBuffer *cmdBuf, const Texture *dstTexture, NVN_NOESCAPE const TextureView *dstView, NVN_NOESCAPE const CopyRegion *dstRegion, NVN_NOESCAPE const uint32_t *color, int mask);
typedef void (NVNAPIENTRY *CommandBufferUpdateUniformBufferFunc) (CommandBuffer *cmdBuf, BufferAddress buffer, size_t bufferSize, ptrdiff_t updateOffset, size_t updateSize, NVN_NOESCAPE const void *data);
...
typedef void (NVNAPIENTRY *CommandBufferSetRenderTargetsFunc) (CommandBuffer *cmdBuf, int numColors, NVN_NOESCAPE const Texture * const *colors, NVN_NOESCAPE const TextureView * const *colorViews, const Texture *depthStencil, NVN_NOESCAPE const TextureView *depthStencilView);
...
typedef void (NVNAPIENTRY *CommandBufferDownsampleTextureViewFunc) (CommandBuffer *cmdBuf, const Texture *src, NVN_NOESCAPE const TextureView *srcView, const Texture *dst, NVN_NOESCAPE const TextureView *dstView);
typedef void (NVNAPIENTRY *CommandBufferTiledDownsampleTextureViewFunc) (CommandBuffer *cmdBuf, const Texture *src, NVN_NOESCAPE const TextureView *srcView, const Texture *dst, NVN_NOESCAPE const TextureView *dstView);
...
typedef void (NVNAPIENTRY *CommandBufferDrawTextureFunc) (CommandBuffer *cmdBuf, TextureHandle texture, NVN_NOESCAPE const DrawTextureRegion *dstRegion, NVN_NOESCAPE const DrawTextureRegion *srcRegion);
...
typedef void (NVNAPIENTRY *CommandBufferSetProgramSubroutinesFunc) (CommandBuffer *cmdBuf, Program *program, ShaderStage::Enum stage, const int first, const int count, NVN_NOESCAPE const int *values);
typedef void (NVNAPIENTRY *CommandBufferBindCoverageModulationTableFunc) (CommandBuffer *cmdBuf, NVN_NOESCAPE const float *entries);
...
typedef void (NVNAPIENTRY *CommandBufferPushDebugGroupDynamicFunc) (CommandBuffer *cmdBuf, uint32_t domainId, NVN_NOESCAPE const char * description);
...
typedef void (NVNAPIENTRY *CommandBufferInsertDebugMarkerDynamicFunc) (CommandBuffer *cmdBuf, uint32_t domainId, NVN_NOESCAPE const char * description);
...
typedef const MemoryPool * (NVNAPIENTRY *EventBuilderGetMemoryPoolFunc) (const EventBuilder *builder);
typedef int64_t (NVNAPIENTRY *EventBuilderGetMemoryOffsetFunc) (const EventBuilder *builder);
...
typedef const MemoryPool * (NVNAPIENTRY *EventGetMemoryPoolFunc) (const Event *event);
typedef int64_t (NVNAPIENTRY *EventGetMemoryOffsetFunc) (const Event *event);
...
typedef void (NVNAPIENTRY *CommandBufferOverrideSubtileSizeFunc) (CommandBuffer *cmdBuf, NVNboolean override, int subtileSize);
typedef NVNboolean (NVNAPIENTRY *CommandBufferInsertCheckpointStaticFunc) (CommandBuffer *cmdBuf, const void *markerData, size_t markerDataSize);
typedef NVNboolean (NVNAPIENTRY *CommandBufferInsertCheckpointDynamicFunc) (CommandBuffer *cmdBuf, NVN_NOESCAPE const void *markerData, size_t markerDataSize);
typedef void (NVNAPIENTRY *CommandBufferSetZCullZF32CompressionEnableFunc) (CommandBuffer *cmdBuf, NVNboolean enable);
typedef NVNboolean (NVNAPIENTRY *DeviceSetAutomaticCheckpointsEnableFunc) (Device *device, NVNboolean enable);
typedef NVNboolean (NVNAPIENTRY *DeviceSetAutomaticCheckpointsMaskFunc) (Device *device, int mask);
typedef NVNboolean (NVNAPIENTRY *DeviceSetAutomaticCheckpointsSamplingIntervalFunc) (Device *device, int interval);
typedef NVNboolean (NVNAPIENTRY *DeviceSetAutomaticCheckpointsByDebugGroupFunc) (Device *device, uint32_t domainId, const char * debugGroupName);
typedef NVNboolean (NVNAPIENTRY *DeviceSetCheckpointCallStacksEnableFunc) (Device *device, NVNboolean enable);
typedef NVNboolean (NVNAPIENTRY *DeviceSetCheckpointMaxCallStacksDepthFunc) (Device *device, int depth);
